...
| 成果 | 指標 | 目標 | 人による確認 |
|------|------|------|:-----------:|
| 容量超過の予防 | 使用率が閾値超過のまま放置された日数 | 0 日 | ✅ |
| 過剰プロビジョニング削減 | `MonthlyCostDeltaUSD` の累計 | 負の値（削減） | ✅ |
| 使用率の可視化 | メトリクスを取得できたボリュームの割合 | 100% | — |
| 推奨の妥当性 | 推奨のうち採用されたものの割合 | > 60% | ✅ |
| 実行の安定性 | ワークフロー成功率 | > 99% | — |

> **目標値の根拠**: いずれも運用上の目安であり、ベンチマーク値ではありません。`MonthlyCostDeltaUSD` はテンプレートのパラメータで与えた単価に基づく試算であり、AWS の請求額ではありません。
