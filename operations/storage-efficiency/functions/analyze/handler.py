...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    for fs_data in file_systems:
        ...
        summary = {
            ...
            "volumes_with_both_enabled": sum(
                1 for v in volumes if v.get("dedupe_enabled") and v.get("compression_enabled")
            ),
            "volumes_with_none": sum(
                1 for v in volumes if not v.get("dedupe_enabled") and not v.get("compression_enabled")
            ),
            ...
            "total_compression_savings_gb": round(
                sum(v.get("compression_savings_bytes", 0) for v in volumes) / (1024**3), 1
            ),
