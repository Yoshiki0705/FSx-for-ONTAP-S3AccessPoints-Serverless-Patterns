...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    for analysis in analyses:
        ...
        cw_client.put_metric_data(Namespace=CW_NAMESPACE, MetricData=[
            {"MetricName": "QoSPoliciesWithLimits", "Value": summary.get("policies_with_limits", 0), "Unit": "Count", "Dimensions": [{"Name": "FileSystemId", "Value": fs_id}]},
            {"MetricName": "VolumesWithoutQoS", "Value": summary.get("volumes_without_qos", 0), "Unit": "Count", "Dimensions": [{"Name": "FileSystemId", "Value": fs_id}]},
            {"MetricName": "QoSRecommendationCount", "Value": summary.get("recommendation_count", 0), "Unit": "Count", "Dimensions": [{"Name": "FileSystemId", "Value": fs_id}]},
        ])

        alert_required = automation_level >= 1 and len(recommendations) > 0
        if alert_required and alert_topic_arn:
            sns = boto3.client("sns")
            sns.publish(TopicArn=alert_topic_arn, Subject=f"[OPS6] QoS Alert - {fs_id}", Message=f"QoS recommendations: {len(recommendations)}")
