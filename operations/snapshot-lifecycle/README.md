...
| 成果 | 指標 | 目標 | 人による確認 |
|------|------|------|:-----------:|
| 保持コンプライアンス | `RetentionCompliancePercent` | 100% | ✅ |
| 不要スナップショットの削減 | `ExpiredSnapshotSizeGB` | 減少傾向 | ✅ |
| ポリシー逸脱の解消 | `PolicyDriftVolumeCount` | 0 | ✅ |
| 検出の即応性 | 逸脱発生から検出までの時間 | スケジュール間隔以内 | — |
| 実行の安定性 | ワークフロー成功率 | > 99% | — |

> **コンプライアンス率の定義**: `RetentionCompliancePercent` は「設定された保持ポリシーに一致しているボリュームの割合」です。**法令上の保持要件を満たしているかの判定ではありません。** 保持期間そのものが法令に適合しているかは、組織のコンプライアンス部門の確認事項です。
