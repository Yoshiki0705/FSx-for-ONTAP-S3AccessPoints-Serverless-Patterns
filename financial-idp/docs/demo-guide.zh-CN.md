...
| # | 数据 | 用途 |
|---|--------|------|
| 1 | 发票 PDF（10 件） | 主要处理对象 |
| 2 | 合同 PDF（3 件） | 文档分类演示 |
| 3 | 收据图像（3 件） | 图像 OCR 演示 |
| 4 | 低质量扫描（2 件） | 可信度评估演示 |
...
| Lambda (OCR Processor) | 通过 Textract 进行文档文本提取 |
| Lambda (Entity Extractor) | 通过 Bedrock 进行实体提取 |
...
![UC2 Step Functions 图表视图 (SUCCEEDED)](../../docs/screenshots/masked/uc2-demo/step-functions-graph-succeeded.png)
...
4. **遮罩处理**:
   - 通过 `python3 scripts/mask_uc_demos.py financial-idp-demo` 自动遮罩
