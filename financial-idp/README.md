...
| メトリクス | 測定方法 |
|-----------|--------|
| 請求書処理件数 / 時間 | CloudWatch Metrics / Step Functions 実行結果 |
| OCR 精度（正確に抽出されたフィールド率） | CloudWatch Metrics / Step Functions 実行結果 |
| 手動データ入力工数の削減率 | CloudWatch Metrics / Step Functions 実行結果 |
| 処理あたりのコスト | CloudWatch Metrics / Step Functions 実行結果 |
| Human Review が必要な件数の割合 | CloudWatch Metrics / Step Functions 実行結果 |
