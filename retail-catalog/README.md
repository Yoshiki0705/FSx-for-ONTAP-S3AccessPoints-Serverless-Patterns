...
| メトリクス | 測定方法 |
|-----------|--------|
| 商品画像処理件数 / 実行 | CloudWatch Metrics / Step Functions 実行結果 |
| ラベル検出精度（Rekognition） | CloudWatch Metrics / Step Functions 実行結果 |
| カタログメタデータ生成時間 | CloudWatch Metrics / Step Functions 実行結果 |
| 手動タグ付け工数の削減率 | CloudWatch Metrics / Step Functions 実行結果 |
