"""小売 / EC Discovery Lambda ハンドラ

共通 Discovery Lambda パターンを継承し、商品画像ファイルを検出する。
S3 Access Point からオブジェクト一覧を取得し、.jpg, .jpeg, .png, .webp
サフィックスフィルタで商品画像ファイルを抽出する。
...
Environment Variables:
    ...
    SUFFIX_FILTER: サフィックスフィルタ (optional, デフォルト: .jpg,.jpeg,.png,.webp)
...
# 商品画像のデフォルトサフィックス
DEFAULT_SUFFIXES = [".jpg", ".jpeg", ".png", ".webp"]
...
def handler(event, context):
    """小売 / EC Discovery Lambda

    S3 AP から商品画像ファイルの一覧を取得し、
    ...
    logger.info(
        "Retail Catalog Discovery started: access_point=%s, prefix=%r, suffixes=%s",
    ...
    for suffix in suffixes:
        with xray_subsegment(
            ...
            annotations={"service_name": "s3", "operation": "ListObjectsV2", "use_case": "retail-catalog"},
    ...
    logger.info(
        "Retail Catalog Discovery completed: total_objects=%d, manifest=%s",
        len(unique_objects),
        manifest_key,
    )
