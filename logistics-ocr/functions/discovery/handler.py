"""物流 / サプライチェーン Discovery Lambda ハンドラ

共通 Discovery Lambda パターンを継承し、配送伝票画像と倉庫在庫画像を検出する。
S3 Access Point からオブジェクト一覧を取得し、配送伝票 (.jpg, .jpeg, .png, .tiff, .pdf)
と倉庫在庫画像 (.jpg, .jpeg, .png) をサフィックスフィルタで抽出する。
...
Environment Variables:
    ...
    SUFFIX_FILTER: サフィックスフィルタ (optional, デフォルト: .jpg,.jpeg,.png,.tiff,.pdf)
...
# 配送伝票 + 倉庫在庫画像のデフォルトサフィックス
DEFAULT_SUFFIXES = [".jpg", ".jpeg", ".png", ".tiff", ".pdf"]

# 倉庫在庫画像のサフィックス（物体検出対象）
INVENTORY_IMAGE_SUFFIXES = [".jpg", ".jpeg", ".png"]
...
def handler(event, context):
    ...
    S3 AP から配送伝票画像 (.jpg, .jpeg, .png, .tiff, .pdf) と
    倉庫在庫画像 (.jpg, .jpeg, .png) の一覧を取得し、
    ...
    logger.info(
        "Logistics OCR Discovery started: access_point=%s, prefix=%r, suffixes=%s",
    ...
    logger.info(
        "Logistics OCR Discovery completed: total=%d, slips=%d, inventory=%d, manifest=%s",
        len(unique_objects),
        len(slip_objects),
