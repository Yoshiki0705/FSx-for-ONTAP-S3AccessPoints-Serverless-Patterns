...
| **シナリオ B: 自動化** | A の手動作業を Lambda + Step Functions で自動化 | EventBridge + Step Functions が自動実行 |

**デモの核心メッセージ**: 「AI に使わせたいデータは、この Windows フォルダーに置いて、自分で
メンテナンスしてください」── 最初は手動で体験し、次にその作業をサーバーレスで自動化する。

**想定時間**: 10〜12 分（A: 5〜6 分 + B: 5〜6 分）
...
| シナリオ A（手動） | シナリオ B（自動化） |
|-------------------|---------------------|
| ① エクスプローラーで投入 | ① エクスプローラーで投入（同じ） |
| ② コンソール/CLI で手動同期 | ② Auto-Sync Lambda が差分検知して自動起動 |
| ③ get-ingestion-job で完了確認 | ③ Ingestion Status Lambda がポーリングで完了判定 |
| ④ 人が結果を確認 | ④ SNS が結果を自動通知 |

→ 業務ユーザーの操作（①）は両シナリオで不変。**変わるのは②〜④を人がやるか、サーバーレスがやるか**だけ。
...
| コンポーネント | 役割 | 使用シナリオ |
|--------------|------|-------------|
| FSx ONTAP AI 専用ボリューム | SMB 共有、業務ユーザーが直接維持 | A / B |
| S3 Access Point | AI 取り込み読み取りパス | A / B |
| Bedrock Knowledge Base | マネージド RAG | A / B |
| Query Lambda | RetrieveAndGenerate | A / B |
...
| EventBridge Scheduler | 定期自動実行 | B |
| SNS | 結果通知 | B |
