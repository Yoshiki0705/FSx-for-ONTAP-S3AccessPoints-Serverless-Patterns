...
{
  "source": "fsxn.fpolicy",
  "detail-type": "FPolicyFileEvent",
  "detail": {
    "operation": "CREATE",
    "path": "/ai_knowledge_smb/sales/product-catalog/product-x-spec.pdf",
    "volume": "ai_knowledge_smb",
    "svm": "FPolicySMB",
    "user": "UC29DEMO\\sales-user",
...
EventBridge ルールが `ai-knowledge/` プレフィックスに一致するイベントのみをフィルタし、
...
| FPolicy ポリシー | ai-knowledge ボリュームの CREATE/WRITE/DELETE/RENAME を監視 |
| Persistent Store | FPolicy サーバーダウンタイム中のイベントを保全（ONTAP 機能） |
| EventBridge ルール | `ai-knowledge/` プレフィックスフィルタ + KB Trigger Lambda ターゲット |
| デバウンス | 短時間に大量ファイル投入時のバースト制御（連続トリガーの抑制） |
