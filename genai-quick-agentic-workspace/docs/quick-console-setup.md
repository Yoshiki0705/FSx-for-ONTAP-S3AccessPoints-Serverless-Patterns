...
Amazon Quick を有効化し、Quick Knowledge の **Amazon S3 コネクタ** で FSxN S3 Access Point への接続を試行した結果:
...
2. しかし接続検証で **「You do not have permissions to access the S3 bucket.」** となり、
   Quick のデータアクセスロール（同一アカウント・`aws-quicksight-service-role-v0` 等）に S3 AP アクセス権が必要。
3. **FSxN S3 AP のデュアルレイヤー認証**により、IAM 識別子ポリシー付与だけでは不十分。さらに
   **S3 AP リソースポリシーへの principal 追加を試みたが `MalformedPolicy: Invalid principal in policy`** となり、
   標準手順では素直に通らなかった（FSxN S3 AP のポリシー principal 制約）。

> 画面証跡: `docs/screenshots/masked/quick-s3-kb-connect.png`（エイリアス受理＋権限メッセージ）

### Remediation / 推奨

- **本命の RAG 経路は Bedrock Knowledge Base（UC29）を使う**。FSxN S3 AP → Bedrock KB は実機で取り込み COMPLETE 済み。
  Quick からは Bedrock KB を参照する構成（または Quick の RAG を別途）に寄せる。
- Quick Index で純粋な S3 を使いたい場合は、**通常の S3 バケットにステージング**（FSxN から S3 へ同期）して接続するのが確実。
- FSxN S3 AP を Quick から直接使う場合は、AWS の
  ...
  と FSxN S3 AP のデュアルレイヤー認証要件（IAM + AP リソースポリシー + FSx identity）を満たすロール/ポリシー設計が必要。
