...
- **Action APIは認証（IAM/SigV4）**: 認証なしの公開エンドポイントにしない。ただし**認証≠認可**であり、
  どの呼び出し元がどのアクションを実行できるかのきめ細かい認可は利用側の責務。本実装は監査フィールド
  （`requested_by`/`created_by`）を**本文ではなく認証済み呼び出し元**から設定し、なりすましを防ぐ
- **human-in-the-loop（非強制スタブ）**: `request_approval`は承認**要求**を記録・通知する入口であり、
  高リスク操作の実行を技術的にゲートするものではない（`enforced: false`）。本番で強制的なHITLが必要な場合は
  承認ストア（例: DynamoDB）+ 実行側の承認検証を追加すること
