"""ゲノミクス / バイオインフォマティクス Discovery Lambda ハンドラ

共通 Discovery Lambda パターンを継承し、ゲノミクスデータファイルを検出する。
S3 Access Point からオブジェクト一覧を取得し、.fastq, .fastq.gz, .bam,
.vcf, .vcf.gz サフィックスフィルタでゲノミクスファイルを抽出する。
...
Environment Variables:
    ...
    SUFFIX_FILTER: サフィックスフィルタ (optional, デフォルト: .fastq,.fastq.gz,.bam,.vcf,.vcf.gz)
...
# ゲノミクスデータファイルのデフォルトサフィックス
DEFAULT_SUFFIXES = [".fastq", ".fastq.gz", ".bam", ".vcf", ".vcf.gz"]
...
def handler(event, context):
    """ゲノミクス / バイオインフォマティクス Discovery Lambda

    S3 AP から FASTQ/BAM/VCF ゲノミクスファイルの一覧を取得し、
    ...
    logger.info(
        "Genomics Discovery started: access_point=%s, prefix=%r, suffixes=%s",
    ...
    for suffix in suffixes:
        with xray_subsegment(
            ...
            annotations={"service_name": "s3", "operation": "ListObjectsV2", "use_case": "genomics-pipeline"},
    ...
    logger.info(
        "Genomics Discovery completed: total_objects=%d, manifest=%s",
        len(unique_objects),
        manifest_key,
    )

    # ファイルタイプ別に分類（Step Functions Map ステート用）
    fastq_objects = [o for o in unique_objects if any(o["Key"].endswith(s) for s in [".fastq", ".fastq.gz"])]
    bam_objects = [o for o in unique_objects if o["Key"].endswith(".bam")]
    vcf_objects = [o for o in unique_objects if any(o["Key"].endswith(s) for s in [".vcf", ".vcf.gz"])]
    ...
    return {
        ...
        "bam_objects": bam_objects,
        "vcf_objects": vcf_objects,
        "metadata": metadata,
    }
