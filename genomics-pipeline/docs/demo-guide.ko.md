...
- 📸 **UI/UX 촬영**: ⚠️ SFN 구조만 촬영 (Phase 8 Theme D, commit c66084f — IAM S3AP 권한 부족으로 FAILED, Theme I에서 수정 예정)

### 기존 스크린샷 (Phase 1-6에서 해당분)

![UC7 Step Functions 그래프 (워크플로우 구조)](../../docs/screenshots/masked/uc7-demo/step-functions-graph-structure.png)

### 재검증 시 UI/UX 대상 화면 (권장 촬영 목록)

- S3 출력 버킷 (fastq-qc/, variant-summary/, entities/)
- Athena 쿼리 결과 (변이 빈도 집계)
- Comprehend Medical 엔티티 (유전자, 질병, 돌연변이)
- Bedrock 생성 연구 보고서
