...
│                                                                              │
│  /vol/genomics_data/                                                         │
│  ├── fastq/sample_001/R1.fastq.gz          (FASTQ シーケンスデータ)          │
│  ├── fastq/sample_001/R2.fastq.gz          (FASTQ シーケンスデータ)          │
│  ├── bam/sample_001/aligned.bam            (BAM アライメントデータ)          │
│  ├── vcf/sample_001/variants.vcf.gz        (VCF バリアントコール)            │
│  └── vcf/sample_002/variants.vcf           (VCF バリアントコール)            │
│                                                                              │
└──────────────────────────────────┬───────────────────────────────────────────┘
                                   │
                                   ▼
...
│  • ListObjectsV2 (FASTQ/BAM/VCF ファイル検出)                               │
│  • GetObject (ファイル取得 — ストリーミングダウンロード)                      │
...
│  │             │    │                      │    │                       │  │
│  │  • VPC内     │    │  • ストリーミング     │    │  • VCF パース          │  │
│  │  • S3 AP List│    │  • Q30 スコア        │    │  • SNP/InDel 集計      │  │
│  │  • FASTQ/VCF │    │  • GC 含量           │    │  • Ti/Tv 比算出        │  │
│  └─────────────┘    └──────────────────────┘    └────────────────────────┘  │
│                                                         │                    │
│                                                         ▼                    │
│                      ┌──────────────────────┐    ┌────────────────────┐      │
...
│  │   ├── sample_001_qc.json                ← 品質メトリクス                 │
│  │   └── sample_002_qc.json                                                  │
│  ├── variant-stats/YYYY/MM/DD/                                               │
│  │   ├── sample_001_variants.json          ← バリアント統計                 │
│  │   └── sample_002_variants.json                                            │
│  ├── athena-results/                                                         │
│  │   └── {query-execution-id}.csv          ← 品質閾値分析結果               │
│  └── reports/YYYY/MM/DD/                                                     │
│      └── research_summary.md               ← 研究サマリーレポート           │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
