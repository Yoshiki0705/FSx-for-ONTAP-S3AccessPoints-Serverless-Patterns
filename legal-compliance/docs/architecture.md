...
│  • ListObjectsV2 (ファイル一覧取得)                                          │
│  • ONTAP REST API (ACL / セキュリティ情報取得)                               │
...
│  │  • ONTAP API │    │  • JSON Lines 出力   │    │  • 過剰権限検出│          │
│  └─────────────┘    └──────────────────────┘    └───────┬────────┘          │
│                                                          │                   │
│                                                          ▼                   │
│                                                 ┌────────────────┐          │
...
│  ├── acl-data/YYYY/MM/DD/                                                    │
│  │   ├── shared_data_acl.jsonl      ← ACL 情報 (JSON Lines)                 │
│  │   └── metadata.json              ← ボリューム・共有メタデータ             │
│  ├── athena-results/                                                         │
│  │   └── {query-execution-id}.csv   ← 違反検出結果                          │
│  └── reports/YYYY/MM/DD/                                                     │
│      └── compliance-report-{id}.md  ← Bedrock コンプライアンスレポート       │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
