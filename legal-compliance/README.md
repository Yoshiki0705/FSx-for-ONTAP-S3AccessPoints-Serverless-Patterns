...
| メトリクス | 測定方法 |
|-----------|--------|
| スキャン対象ファイル数 / 実行あたり | CloudWatch Metrics / Step Functions 実行結果 |
| 過剰権限検出数 / スキャン | CloudWatch Metrics / Step Functions 実行結果 |
| コンプライアンスレポート生成時間 | CloudWatch Metrics / Step Functions 実行結果 |
| 手動監査工数の削減率 | CloudWatch Metrics / Step Functions 実行結果 |
