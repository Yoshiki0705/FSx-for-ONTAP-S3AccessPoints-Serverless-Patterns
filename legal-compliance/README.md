...
> **注記**: 上記は小規模サンプルランの結果であり、本番環境のスループット見積もりや性能保証ではありません。UC1 の sample run は合成または非機微のサンプルファイルを使用しており、顧客の法務文書を表すものではありません。本 sample run は処理パスの検証のみです。法的妥当性、分類品質、レビュー完全性は、顧客固有の PoC で別途評価してください。
