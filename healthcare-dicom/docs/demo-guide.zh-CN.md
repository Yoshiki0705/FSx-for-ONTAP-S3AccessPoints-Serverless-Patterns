...
| **日常业务** | 医疗影像管理、研究数据提供、隐私保护 |
| **课题** | 大量 DICOM 文件的手动匿名化耗时且存在错误风险 |
| **期望成果** | 安全可靠的匿名化和审计追踪自动化 |

### Persona：高桥（临床研究数据管理员）
...
**解说要点**:
> 为多中心合作研究需要匿名化 10,000 个 DICOM 文件。手动处理存在错误风险，不允许个人信息泄露。
...
**解说要点**:
> 指定匿名化对象数据集，启动匿名化工作流程。设置匿名化规则（删除、哈希化、泛化）。

**Key Visual**：工作流程启动、匿名化规则设置画面
...
**解说要点**:
> 自动验证匿名化后的文件。扫描所有标签检查是否有残留的个人信息。同时确认影像的完整性。
...
**解说要点**:
> 自动生成匿名化处理的审计报告。记录处理件数、删除标签数、验证结果。可作为研究伦理委员会的提交资料使用。
...
- 📸 **UI/UX 拍摄**：✅ SFN Graph 完成（Phase 8 Theme D, commit c66084f）
...
1. **事前准备**:
   - 使用 `bash scripts/verify_phase7_prerequisites.sh` 确认前提（共享 VPC/S3 AP 有无）
...
4. **掩码处理**:
   - 使用 `python3 scripts/mask_uc_demos.py healthcare-dicom-demo` 自动掩码
   - 根据 `docs/screenshots/MASK_GUIDE.md` 进行追加掩码（如需要）
