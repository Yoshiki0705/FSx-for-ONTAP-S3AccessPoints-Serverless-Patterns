...
| FC7 | [`devops-flexclone-cicd/`](devops-flexclone-cicd/) | FlexClone Dev/Test リフレッシュ & CI/CD |
...
| [`content-edge-delivery/`](content-edge-delivery/) | CDN/エッジ配信パターン（ベンダー非依存・CloudFront/サードパーティ、[CDN比較](docs/cdn-comparison.md)） |
...
本リポジトリは **28 の業界別ユースケース** + **イベント駆動 FPolicy パターン** + **7 つの FlexCache/FlexClone パターン** + **コンテンツエッジ配信パターン** を含むサーバーレスパターンライブラリです。
...
|-------------|:---:|:---:|:---:|
| **テキスト/PDF** | [UC1 法務](legal-compliance/) / [UC13 教育](education-research/) / [UC24 NPO](nonprofit-grant-management/) | [UC2 金融IDP](financial-idp/) / [UC16 政府](government-archives/) / [UC27 HR](hr-document-screening/) | [UC1 法務](legal-compliance/) |
| **画像** | [UC11 小売](retail-catalog/) / [UC14 保険](insurance-claims/) / [UC19 広告](adtech-creative-management/) | [UC5 医療DICOM](healthcare-dicom/) / [UC3 製造](manufacturing-analytics/) / [UC25 電力](utilities-asset-inspection/) | [UC15 衛星](defense-satellite/) / [UC21 農業](agri-food-traceability/) |
| **構造化データ (CSV/JSON)** | [SAP](sap-erp-adjacent/) / [UC12 物流](logistics-ocr/) / [UC18 通信](telecom-network-analytics/) | [UC3 製造](manufacturing-analytics/) / [UC8 エネルギー](energy-seismic/) / [UC23 ESG](sustainability-esg-reporting/) | [UC7 ゲノミクス](genomics-pipeline/) / [UC28 化学](chemical-sds-management/) |
...
| **GIS/地理空間** | — | [UC17 スマートシティ](smart-city-geospatial/) / [UC21 農業](agri-food-traceability/) | [UC17 スマートシティ](smart-city-geospatial/) |
| **ネットワーク/ログ** | [UC18 通信CDR](telecom-network-analytics/) | [UC22 運輸保守](transportation-maintenance/) | — |
| **文書/契約** | [UC20 旅行](travel-document-processing/) / [UC26 不動産](real-estate-portfolio/) | [UC28 化学SDS](chemical-sds-management/) | — |

**FlexCache/FlexClone が必要な場合**: [FC1 AnyCast/DR](flexcache-anycast-dr/) | [FC2 動的レンダリング](dynamic-flexcache-render-workflow/) | [FC3 RAG](genai-rag-enterprise-files/) | [FC7 Dev/Test & CI/CD](devops-flexclone-cicd/)

**CDN/エッジ配信が必要な場合**: [content-edge-delivery](content-edge-delivery/)（CloudFront/Akamai/Fastly/Cloudflare ほか、[CDN比較](docs/cdn-comparison.md)）
...
### 60 分パス: PoC を実行する
- 1 つの UC テンプレートをデプロイ（例: [UC1 legal-compliance](legal-compliance/README.md)）
- S3 Access Point を設定し、ListObjectsV2 / GetObject を確認
- CloudWatch メトリクスで実行結果を確認

### 1 日パス: パートナー/顧客ワークショップ
- FPolicy パイプラインをデプロイ（[event-driven-fpolicy/](event-driven-fpolicy/README.md)）
...
│  ・28 業界別サーバーレスパターン + 7 FlexCache パターン                 │
...
| # | ディレクトリ | パターン | 概要 | 状態 |
|---|-------------|---------|------|------|
| FC1 | [`flexcache-anycast-dr/`](flexcache-anycast-dr/README.md) | FlexCache AnyCast / DR | ヘルスチェック・ルート判定・フェイルオーバーシミュレーション | ✅ コード・ドキュメント完了 |
| FC2 | [`dynamic-flexcache-render-workflow/`](dynamic-flexcache-render-workflow/README.md) | Dynamic FlexCache Render/EDA | ジョブ単位 FlexCache 動的作成・削除ワークフロー | ✅ コード・テスト完了 |
| FC3 | [`genai-rag-enterprise-files/`](genai-rag-enterprise-files/README.md) | GenAI RAG over Enterprise Files | 権限ベース RAG（S3 AP 経由、データコピー不要） | ✅ コード・テスト完了 |
| FC4 | [`automotive-cae/`](automotive-cae/README.md) | Automotive CAE Analytics | CAE シミュレーション結果の自動分析 | ✅ コード・テスト完了 |
| FC5 | [`life-sciences-research/`](life-sciences-research/README.md) | Life Sciences Research | 研究データ（画像・シーケンス・論文）の自動分析 | ✅ テンプレート完了 |
| FC6 | [`gaming-build-pipeline/`](gaming-build-pipeline/README.md) | Gaming Build Pipeline | ゲームアセット品質チェック・ログ分析 | ✅ テンプレート完了 |
...
| UC18 | 通信 (CDR/ネットワーク) | [📐 Architecture](telecom-network-analytics/docs/architecture.md) | [🎬 Demo Guide](telecom-network-analytics/docs/demo-guide.md) |
| UC19 | 広告 (クリエイティブ) | [📐 Architecture](adtech-creative-management/docs/architecture.md) | [🎬 Demo Guide](adtech-creative-management/docs/demo-guide.md) |
| UC20 | 旅行 (予約文書) | [📐 Architecture](travel-document-processing/docs/architecture.md) | [🎬 Demo Guide](travel-document-processing/docs/demo-guide.md) |
| UC21 | 農業・食品 (トレーサビリティ) | [📐 Architecture](agri-food-traceability/docs/architecture.md) | [🎬 Demo Guide](agri-food-traceability/docs/demo-guide.md) |
| UC22 | 運輸・鉄道 (保守点検) | [📐 Architecture](transportation-maintenance/docs/architecture.md) | [🎬 Demo Guide](transportation-maintenance/docs/demo-guide.md) |
| UC23 | サステナビリティ (ESG) | [📐 Architecture](sustainability-esg-reporting/docs/architecture.md) | [🎬 Demo Guide](sustainability-esg-reporting/docs/demo-guide.md) |
| UC24 | NPO (助成金) | [📐 Architecture](nonprofit-grant-management/docs/architecture.md) | [🎬 Demo Guide](nonprofit-grant-management/docs/demo-guide.md) |
| UC25 | 電力 (設備点検) | [📐 Architecture](utilities-asset-inspection/docs/architecture.md) | [🎬 Demo Guide](utilities-asset-inspection/docs/demo-guide.md) |
...
| — | コンテンツエッジ配信 (CDN) | [📐 Architecture](content-edge-delivery/docs/architecture.md) | [🎬 Demo Guide](content-edge-delivery/docs/demo-guide.md) |
...
| UC18 | 通信・ネットワーク | 未掲載 | CDR 分析結果 / 異常検知アラート / ネットワーク健全性レポート（NW 運用担当者向け、今後撮影予定） | [`telecom-network-analytics/docs/demo-guide.md`](telecom-network-analytics/docs/demo-guide.md) |
| UC19 | 広告・クリエイティブ | 未掲載 | ブランドコンプライアンスチェック結果 / タグ付け結果（広告運用担当者向け、今後撮影予定） | [`adtech-creative-management/docs/demo-guide.md`](adtech-creative-management/docs/demo-guide.md) |
| UC20 | 旅行・予約文書 | 未掲載 | 予約文書 OCR 結果 / 施設点検レポート（旅行業務担当者向け、今後撮影予定） | [`travel-document-processing/docs/demo-guide.md`](travel-document-processing/docs/demo-guide.md) |
| UC21 | 農業・食品 | 未掲載 | 航空画像分析 / トレーサビリティ文書（農業技術者向け、今後撮影予定） | [`agri-food-traceability/docs/demo-guide.md`](agri-food-traceability/docs/demo-guide.md) |
| UC22 | 運輸・鉄道 | 未掲載 | 設備点検画像 / 保守レポート（保守担当者向け、今後撮影予定） | [`transportation-maintenance/docs/demo-guide.md`](transportation-maintenance/docs/demo-guide.md) |
| UC23 | サステナビリティ | 未掲載 | ESG メトリクス / コンプライアンスレポート（ESG 担当者向け、今後撮影予定） | [`sustainability-esg-reporting/docs/demo-guide.md`](sustainability-esg-reporting/docs/demo-guide.md) |
| UC24 | NPO・助成金 | 未掲載 | 申請分類結果 / 成果マッチング（助成金担当者向け、今後撮影予定） | [`nonprofit-grant-management/docs/demo-guide.md`](nonprofit-grant-management/docs/demo-guide.md) |
| UC25 | 電力・設備点検 | 未掲載 | ドローン画像分析 / SCADA ログ（設備管理者向け、今後撮影予定） | [`utilities-asset-inspection/docs/demo-guide.md`](utilities-asset-inspection/docs/demo-guide.md) |
| UC26 | 不動産 | 未掲載 | 物件画像分析 / 契約書データ抽出（不動産担当者向け、今後撮影予定） | [`real-estate-portfolio/docs/demo-guide.md`](real-estate-portfolio/docs/demo-guide.md) |
| UC27 | 人材・HR | 未掲載 | 履歴書スクリーニング結果 / 候補者評価（人事担当者向け、今後撮影予定） | [`hr-document-screening/docs/demo-guide.md`](hr-document-screening/docs/demo-guide.md) |
| UC28 | 化学・素材 | 未掲載 | SDS 管理結果 / ラボノート分析（研究開発担当者向け、今後撮影予定） | [`chemical-sds-management/docs/demo-guide.md`](chemical-sds-management/docs/demo-guide.md) |
