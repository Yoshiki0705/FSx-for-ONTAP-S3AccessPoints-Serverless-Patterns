...
（リポジトリ全体で 146 箇所 / 10 ファイル）。そのため次のような退行を誰も検出
できなかった:
...
**ここで検出できないもの**: `os.environ[...]` の必須キーとテンプレートの不一致。
ハーネスが `DEFAULT_ENV` を自分で注入するため、テンプレート側の欠落は隠れる。
そちらは `test_template_env_contract.py` が担当する。
