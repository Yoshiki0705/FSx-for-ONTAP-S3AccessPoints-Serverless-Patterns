...
class TestDetermineInferencePath:
    ...
    def test_inference_type_none_returns_batch_transform(self):
        ...
        result = determine_inference_path(file_count=5, batch_threshold=10, inference_type="none")
    ...
    def test_inference_type_none_ignores_file_count(self):
        ...
        result = determine_inference_path(file_count=1, batch_threshold=100, inference_type="none")
        assert result == InferencePath.BATCH_TRANSFORM

        # file_count >= threshold でも BATCH_TRANSFORM
        result = determine_inference_path(file_count=100, batch_threshold=10, inference_type="none")
        assert result == InferencePath.BATCH_TRANSFORM
    ...
    def test_inference_type_serverless_returns_serverless_inference(self):
        ...
        result = determine_inference_path(file_count=5, batch_threshold=10, inference_type="serverless")
    ...
    def test_inference_type_serverless_ignores_file_count(self):
        ...
        result = determine_inference_path(file_count=100, batch_threshold=10, inference_type="serverless")
        assert result == InferencePath.SERVERLESS_INFERENCE

        # file_count=0 でも SERVERLESS_INFERENCE
        result = determine_inference_path(file_count=0, batch_threshold=10, inference_type="serverless")
        assert result == InferencePath.SERVERLESS_INFERENCE
    ...
    def test_file_count_at_threshold_returns_batch_transform(self):
        ...
        result = determine_inference_path(file_count=10, batch_threshold=10, inference_type="provisioned")
        assert result == InferencePath.BATCH_TRANSFORM

    def test_file_count_above_threshold_returns_batch_transform(self):
        """file_count > batch_threshold で BATCH_TRANSFORM を返す"""
        result = determine_inference_path(file_count=50, batch_threshold=10, inference_type="provisioned")
        assert result == InferencePath.BATCH_TRANSFORM
    ...
    def test_file_count_below_threshold_provisioned_returns_realtime(self):
        ...
        result = determine_inference_path(file_count=5, batch_threshold=10, inference_type="provisioned")
    ...
    def test_file_count_zero(self):
        ...
        result = determine_inference_path(file_count=0, batch_threshold=10, inference_type="provisioned")
        assert result == InferencePath.REALTIME_ENDPOINT

        # none: always BATCH_TRANSFORM
        result = determine_inference_path(file_count=0, batch_threshold=10, inference_type="none")
        assert result == InferencePath.BATCH_TRANSFORM

    def test_threshold_equals_one(self):
        """batch_threshold=1 のエッジケース"""
        # file_count=0 < 1 → REALTIME_ENDPOINT
        result = determine_inference_path(file_count=0, batch_threshold=1, inference_type="provisioned")
        assert result == InferencePath.REALTIME_ENDPOINT

        # file_count=1 >= 1 → BATCH_TRANSFORM
        result = determine_inference_path(file_count=1, batch_threshold=1, inference_type="provisioned")
        assert result == InferencePath.BATCH_TRANSFORM

    def test_file_count_equals_threshold_boundary(self):
        """file_count == threshold の境界値テスト（>= なので BATCH_TRANSFORM）"""
        result = determine_inference_path(file_count=100, batch_threshold=100, inference_type="provisioned")
        assert result == InferencePath.BATCH_TRANSFORM
    ...
    def test_invalid_inference_type_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="Invalid inference_type"):
            determine_inference_path(file_count=5, batch_threshold=10, inference_type="invalid")

    def test_empty_inference_type_raises_value_error(self):
        """空文字列の inference_type で ValueError を raise する"""
        with pytest.raises(ValueError, match="Invalid inference_type"):
            determine_inference_path(file_count=5, batch_threshold=10, inference_type="")

    def test_case_sensitive_inference_type(self):
        """inference_type は大文字小文字を区別する"""
        with pytest.raises(ValueError, match="Invalid inference_type"):
            determine_inference_path(file_count=5, batch_threshold=10, inference_type="Serverless")

        with pytest.raises(ValueError, match="Invalid inference_type"):
            determine_inference_path(file_count=5, batch_threshold=10, inference_type="NONE")
...
class TestValidateServerlessConfigValid:
    ...
    def test_all_valid_memory_sizes(self, memory_size: int):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=memory_size, max_concurrency=5)
        assert is_valid is True
        assert error is None

    def test_min_max_concurrency(self):
        """MaxConcurrency=1（最小値）で valid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=1)
        assert is_valid is True
        assert error is None

    def test_max_max_concurrency(self):
        """MaxConcurrency=200（最大値）で valid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=200)
        assert is_valid is True
        assert error is None

    def test_valid_provisioned_concurrency(self):
        """ProvisionedConcurrency=0（デフォルト）で valid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=5, provisioned_concurrency=0)
        assert is_valid is True
        assert error is None

    def test_positive_provisioned_concurrency(self):
        """ProvisionedConcurrency > 0 で valid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=5, provisioned_concurrency=3)
        assert is_valid is True
        assert error is None
...
class TestValidateServerlessConfigInvalid:
    ...
    def test_memory_size_below_minimum(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=512, max_concurrency=5)
        assert is_valid is False
        assert "MemorySizeInMB" in error

    def test_memory_size_above_maximum(self):
        """MemorySizeInMB > 6144 で invalid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=8192, max_concurrency=5)
        assert is_valid is False
        assert "MemorySizeInMB" in error
    ...
    def test_memory_size_not_in_valid_set(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=1500, max_concurrency=5)
        assert is_valid is False
        assert "MemorySizeInMB" in error

    def test_memory_size_zero(self):
        """MemorySizeInMB=0 で invalid を返す"""
        is_valid, error = validate_serverless_config(memory_size_mb=0, max_concurrency=5)
        assert is_valid is False
        assert "MemorySizeInMB" in error
    ...
    def test_max_concurrency_below_minimum(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=0)
        assert is_valid is False
    ...
    def test_max_concurrency_above_maximum(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=201)
        assert is_valid is False
        assert "MaxConcurrency" in error
    ...
    def test_provisioned_concurrency_negative(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=4096, max_concurrency=5, provisioned_concurrency=-1)
        assert is_valid is False
    ...
    def test_memory_validated_before_concurrency(self):
        ...
        is_valid, error = validate_serverless_config(memory_size_mb=999, max_concurrency=999)
        assert is_valid is False
        assert "MemorySizeInMB" in error
...
class TestDetermineInferencePathComponents:
    ...
    def test_inference_type_components_returns_inference_components(self):
        ...
        result = determine_inference_path(file_count=5, batch_threshold=10, inference_type="components")
    ...
    def test_inference_type_components_ignores_file_count(self):
        ...
        result = determine_inference_path(file_count=100, batch_threshold=10, inference_type="components")
        assert result == InferencePath.INFERENCE_COMPONENTS

        # file_count=0 でも INFERENCE_COMPONENTS
        result = determine_inference_path(file_count=0, batch_threshold=10, inference_type="components")
        assert result == InferencePath.INFERENCE_COMPONENTS
    ...
    def test_components_has_higher_priority_than_batch_threshold(self):
        ...
        result = determine_inference_path(file_count=1000, batch_threshold=10, inference_type="components")
        assert result == InferencePath.INFERENCE_COMPONENTS
...
class TestValidateInferenceConfig:
    ...
    def test_provisioned_requires_endpoint_name(self):
        ...
        is_valid, error = validate_inference_config(inference_type="provisioned", endpoint_name="")
        assert is_valid is False
    ...
    def test_provisioned_with_endpoint_name_valid(self):
        ...
        is_valid, error = validate_inference_config(inference_type="provisioned", endpoint_name="my-endpoint")
        assert is_valid is True
        assert error is None

    def test_serverless_requires_endpoint_name(self):
        """inference_type='serverless' は endpoint_name が必須"""
        is_valid, error = validate_inference_config(inference_type="serverless", endpoint_name="")
        assert is_valid is False
        assert "endpoint_name" in error
    ...
    def test_components_requires_endpoint_and_component_name(self):
        ...
        # Both missing
        is_valid, error = validate_inference_config(inference_type="components", endpoint_name="", component_name="")
        assert is_valid is False
        assert "endpoint_name" in error
