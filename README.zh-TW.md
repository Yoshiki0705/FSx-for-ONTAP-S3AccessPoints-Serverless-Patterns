...
- **FSx for NetApp ONTAP**: 已部署的檔案系統
  ...
  - 已啟用 S3 Access Point 的磁碟區（network origin: `internet` 建議）
- **網路**: VPC、私有子網路、路由表
...
| 資源 | 參數 | 預設值 | 月固定費 | 說明 |
|------|------|--------|---------|------|
| Interface VPC Endpoints | `EnableVpcEndpoints` | `false` | ~$28.80 | 用於 Secrets Manager、FSx、CloudWatch、SNS。正式環境建議 `true` |
