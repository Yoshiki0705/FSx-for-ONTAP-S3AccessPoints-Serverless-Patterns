...
- 希望偵測透過 S3 存取點寫入的資料（該路徑上的操作不會觸發 FPolicy 通知，即使使用 `mandatory` 同步政策也不會被阻擋。實測 2026-08-26，ONTAP 9.18.1P3D1 — 請使用 EventBridge Scheduler 輪詢或 ONTAP 原生稽核日誌）
...
- AWS 帳戶及適當的 IAM 權限
- FSx for ONTAP 檔案系統（ONTAP 9.17.1 以上）
- VPC、私有子網路（與 FSx for ONTAP SVM 相同的 VPC）
...
```bash
# 透過 SSH 連線到 FSx for ONTAP SVM 後執行以下命令
...
| `VpcId` | 與 FSx for ONTAP 相同 VPC 的 ID | — | ✅ |
...
| 服務 | 區域限制 |
|---------|-------------|
| FSx for ONTAP | [支援區域列表](https://docs.aws.amazon.com/general/latest/gr/fsxn.html) |
...
- [效能](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/en/domains/performance) — 該模式的決策集中之處
