...
> **파일 수준 ACL 추출 안내**: 기본적으로 ACL 추출은 시뮬레이션 모드로 동작합니다(ONTAP 불필요). 실제 ACL을 추출하려면 `OntapManagementIp` / `OntapSecretName`을 지정하세요. 단, 본 템플릿은 `VpcConfig`를 포함하지 않으므로 프라이빗 ONTAP 관리 LIF에 도달하려면 추가 네트워크 구성이 필요합니다.
