...
### AstraZeneca: マルチエージェントシステム (DAIS 2026)

AstraZeneca は治療領域横断でコマーシャルチームが医薬品データ（構造化 + 非構造化 40 万+ 臨床文書）にアクセスするマルチエージェントシステムを構築した。Supervisor Agent が治療領域別サブエージェントを統括し、権限境界を保持しながら 5 → 20+ エージェントへスケールした。

- **成果**: エージェント 10x スケール（5 PoC → 20+ 本番、50+ 設計済み）
- **アーキテクチャ**: Supervisor Agent + 治療領域別サブエージェント + 構造化データクエリ + 非構造化文書 RAG + 行/列レベルセキュリティ
- **主要教訓**: 権限保持設計、Supervisor 分割 vs エージェント追加の判断基準、Human-in-the-loop テスト、データ品質の重要性
- **FSx for ONTAP との関連**: 大量臨床文書を NAS 共有に保存 → S3 AP で AI パイプラインがアクセス → ACL メタデータを抽出しベクトル DB に伝播 → 治療領域別権限フィルタで検索

本パターン（UC7）は同種の課題（研究文書の AI 分析 + 分類）を FSx for ONTAP S3 AP + AWS Bedrock で解決するアーキテクチャを提供する。マルチエージェント拡張は Step Functions による治療領域別ルーティングで実現可能。
...
- [性能](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/ja/domains/performance) — このパターンで判断が集中するところ
