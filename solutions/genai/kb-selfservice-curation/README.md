...
> **注記**: 以下は特定の企業名・担当者名をマスクした、一般化した運用ストーリーです。
...
- ファイルは FSx for ONTAP 上の **正本のまま**（S3 へのコピー不要）
...
| S3 へのコピーによるデータ二重管理 | S3 AP 経由で FSx for ONTAP の正本を直接データソース化 |
...
## ハイブリッド RAG: 内部文書 + Web 検索 (opt-in, NEW)

> AWS Summit NYC 2026 (2026-06-17) で GA になった **AgentCore Web Search Tool** を統合。

`EnableWebSearch=true` を設定すると、Query Lambda が内部 KB 回答に加えてリアルタイム Web 検索結果で補強した統合回答を生成する。

| モード | 回答ソース | ユースケース |
|--------|-----------|-------------|
| `EnableWebSearch=false` (デフォルト) | 内部文書のみ (FSx for ONTAP → S3 Vectors) | 社内ナレッジ QA |
| `EnableWebSearch=true` | 内部文書 + Web 検索結果 | 最新法規・市場動向・製品比較 |

- Graceful degradation: Web Search が失敗しても内部 KB のみで回答
...
- **データ移動なし**: ファイルは FSx for ONTAP 上の正本のまま、S3 AP 経由で読み取りのみ
...
| FSx for ONTAP + Bedrock RAG チュートリアル | [Build RAG with Bedrock](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/tutorial-build-rag-with-bedrock.html) |
...
| コンポーネント | 月額 |
|--------------|------|
| FSx for ONTAP（既存の AI 専用ボリュームを共有） | 既存環境を共有 |
...
> **注記**: 本パターンのパフォーマンス数値は sizing reference であり、service limit ではありません。実環境での性能は FSx for ONTAP スループットキャパシティ、ファイル数、同時実行ワークロードにより異なります。
...
- [セキュリティ・ガバナンス](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/ja/domains/security-governance) — このパターンで判断が集中するところ
