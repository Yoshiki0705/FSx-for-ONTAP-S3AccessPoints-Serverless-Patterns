...
> **注記**: 特定の企業名・担当者名・チーム名をマスクした、一般化した運用ストーリーです。
...
> 📸 Bedrock Knowledge Base 一覧（UC29 用 KB が ACTIVE）:
>
> ![Bedrock KB list](screenshots/masked/bedrock-kb-list.png)
>
> 📸 KB 詳細画面（S3 AP データソース接続済み）:
>
> ![Bedrock KB detail with S3 AP data source](screenshots/masked/bedrock-kb-detail.png)
...
> 2. AD 参加 + SMB 共有を持つ FSx for ONTAP ボリューム（NTFS / multiprotocol セキュリティスタイル）
...
>
> ![Windows SMB share showing 7 role folders](screenshots/masked/windows-smb-share-roles.png)
>
...
> 📸 Bedrock KB データソース画面（Sync ボタン）:
>
> ![Bedrock KB data source sync button](screenshots/masked/bedrock-kb-datasource-sync.png)
...
> 📸 Step Functions 実行画面（全ステート Succeeded）:
>
> ![Step Functions execution succeeded](screenshots/masked/step-functions-execution-succeeded.png)
...
```
業務ユーザー → Windows ドラッグ&ドロップ → FSx for ONTAP AI 専用ボリューム (S3 AP 付き)
    ↓ (即時検知)
...
| コンポーネント | 役割 | 使用シナリオ |
|--------------|------|-------------|
| FSx for ONTAP AI 専用ボリューム | SMB 共有、業務ユーザーが直接維持 | A / B / C |
| S3 Access Point | AI 取り込み読み取りパス | A / B / C |
| Bedrock Knowledge Base | マネージド RAG | A / B / C |
| Query Lambda | RetrieveAndGenerate | A / B / C |
| Query Lambda (Web Search) | RetrieveAndGenerate + AgentCore Web Search | A / B / C (opt-in) |
...
| SNS | 結果通知 | B / C |
| AgentCore Gateway (us-east-1) | Web Search Tool コネクタ | opt-in |

---

## ハイブリッド RAG: Web Search 統合デモ (opt-in)

`EnableWebSearch=true` でデプロイした場合、Query Lambda は内部 KB 回答に加えて Web 検索結果で補強した回答を返す。

### デモ手順

```bash
# 1. Web Search 有効の Query を実行
aws lambda invoke --function-name <QueryFunctionName> \
  --payload '{"query": "製品Xの競合製品と市場での位置づけは？"}' \
  --cli-binary-format raw-in-base64-out out.json && cat out.json
```
...
{
  "status": "completed",
  "query": "製品Xの競合製品と市場での位置づけは？",
  "answer": "製品Xは... [内部: product-x-spec.pdf] による仕様に加え、[Web: Market Analysis 2026](https://...) によると市場シェアは...",
  "citations": [{"source": "s3://.../sales/product-catalog/product-x-spec.pdf"}],
  "web_citations": [
    {"source": "https://example.com/market-report", "title": "Market Analysis 2026", "type": "web"}
  ],
  "web_search_enabled": true
}
```

> **注記**: Web Search は us-east-1 の AgentCore Gateway を経由するため、レイテンシが 100-200ms 追加される。
> Web Search が失敗した場合は `web_citations` が空配列になり、内部 KB のみの回答が返る（graceful degradation）。
