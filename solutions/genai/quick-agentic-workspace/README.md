...
## Quick の各機能と FSx for ONTAP S3 AP の対応
...
| 課題 | 本パターンによる解決 |
|------|-------------------|
| 業務データが S3 にコピーされ二重管理 | S3 AP で FSx for ONTAP の正本を直接データソース化 |
...
## Web 検索で補強されたブリーフ生成 (opt-in, NEW)

> AWS Summit NYC 2026 (2026-06-17) で GA になった **AgentCore Web Search Tool** を統合。

Action API に新しいアクション `generate_brief_with_web` を追加。社内コンテキストに加えて、リアルタイム Web 検索結果で補強したブリーフを生成する。
...
curl -X POST https://<api-id>.execute-api.ap-northeast-1.amazonaws.com/prod/action \
  ...
  -d '{
    ...
    "params": {
      "title": "2026年Q3 データ保護規制の動向",
      "context": "社内では FISC 安全対策基準に準拠した運用を実施中...",
      "web_query": "data protection regulation 2026 Japan"
    }
  }'
```

| アクション | 回答ソース | 読み取り/書き込み |
|-----------|-----------|-----------------|
| `generate_brief` | 内部コンテキストのみ | 読み取り専用 |
| `generate_brief_with_web` | 内部コンテキスト + Web 検索 | 読み取り専用 |

- `EnableWebSearch=true` + `AgentCoreGatewayId` 設定で有効化
...
- **データ移動なし**: ファイルは FSx for ONTAP 上の正本のまま、S3 AP 経由で読み取り
...
| FSx for ONTAP / S3 AP | 既存環境を共有（S3 AP 追加料金なし） |
...
- FSx for ONTAP のスループットは NFS/SMB/S3AP で共有。SMB 書き込みと Quick の読み取りが同一キャパシティを共有
