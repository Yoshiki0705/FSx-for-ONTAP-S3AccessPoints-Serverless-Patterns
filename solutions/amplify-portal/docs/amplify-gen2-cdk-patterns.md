...
npx ampx sandbox --once
  ...
  │     └── cdk-nag は走らない（CDK_NAG=1 のときだけ。CI は別ジョブで合成して検査）
  ├── cdk deploy (差分のみ反映)
...
2. **検証**: `npx tsc --noEmit` + `npx vitest run` + `npm run build`。cdk-nag は `npm run nag`（CI では Stage 2b がベースラインと比較する）
3. **本番**: `npx ampx pipeline-deploy` (Amplify Hosting の CI/CD パイプライン)

## 関連リファレンス

- [アプリの構築ツールの選択肢](../../../docs/ja/scaffolding-and-backend-toolkit-choices.md) — Nx Plugin for AWS / AWS Blocks / Amplify Gen 2 の位置づけ、実測したリソース数と既定値、このリポジトリが Amplify Gen 2 を使っている理由
