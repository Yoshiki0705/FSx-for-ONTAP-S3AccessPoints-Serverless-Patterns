...
- **回転は行の見た目では確認できない**。サムネイルは 32px の正方形にクロップされるため、縦横比は行に出ない（キャッシュ内のオブジェクトは 108x192 で、回転は適用されている）
...
<img src="screenshots/portal-mobile-thumbnails.png" alt="iPhone のファイル一覧に画像の縮小画像が並んでいる状態" width="360">

5 件の画像に縮小画像が出て、`estimate.pdf` は 📕 のまま。`photo_front.jpg` が白っぽいのは元画像がほぼ単色（240,240,240）だからで、忠実な描画である。
