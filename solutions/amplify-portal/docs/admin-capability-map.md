...
|------|------------|--------------|
| クォータルールの変更（上限の変更） | 削除して作り直す。集計はリセットされる | `PATCH /storage/quota/rules/{uuid}` |
| Qtree の変更（security style、エクスポートポリシー） | 削除して作り直す。中身は失われる | `PATCH /storage/qtrees/{svm.uuid}/{id}` |
| ローカルユーザーのパスワード変更・無効化 | 削除して作り直す | `PATCH /protocols/cifs/local-users/{svm.uuid}/{sid}` |
| 名前マッピングの変更 | 削除して作り直す | `PATCH /name-services/name-mappings/{svm.uuid}/{direction}/{index}` |
| FlexCache のサイズ変更 | ボリュームとしてリサイズ（ボリュームパネル経由） | `PATCH /storage/volumes/{uuid}` |
| 複数 SVM の横断表示 | 単一 SVM（`SVM_NAME`）に固定。他 SVM のボリュームは一覧に出ない | `svm.name` を可変にする。UI に SVM セレクターが必要 |
| SnapMirror 系 5 操作のジョブ確認 | 応答は返るが、失敗したジョブを成功として報告しうる | 別のクライアント抽象を経由しており `http, headers` が未配線 |

> **単一 SVM 固定について**: これは設計上の選択（1 デプロイ = 1 SVM）ですが、実測で困る場面が
> ありました。別 SVM に作った SnapMirror 宛先ボリュームがボリューム一覧に現れず、削除に
> UUID の直接指定が必要でした。複数 SVM を持つファイルシステムを 1 つのポータルから運用する
> なら、横断表示は前提条件になります。変更は全パネルに及ぶため独立した変更として扱います。
