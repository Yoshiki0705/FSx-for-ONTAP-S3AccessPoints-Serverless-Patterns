...
|--------|:---:|:---:|:---:|:---:|
| ボリューム | ○ | ○ | ○ リサイズ | ○ |
| Qtree | ○ | ○ | ○ security style・エクスポートポリシー | ○ |
| クォータ | ○ | ○ | ○ 上限の変更 | ○ |
| ストレージ効率 | ○ | — | — | — |
| FlexCache | ○ | ○ | ○ write-back の有効・無効 | ○ |
| FlexClone | ○ | ○ | ○ 分割 | — |
...
上の表の `—` のうち、「ONTAP 側に手段があるのにポータルから到達できない」ものだけを挙げます。
`—` の一部は ONTAP 自体が提供していない操作（SnapLock 保持期間の短縮など）なので、区別します。

| 不足 | 現在の回避策 | ONTAP 側の手段 |
|------|------------|--------------|
| FlexCache のサイズ変更 | ボリュームとしてリサイズ（ボリュームパネル経由） | `PATCH /storage/volumes/{uuid}` |
| Qtree の名前変更 | 削除して作り直す | `PATCH /storage/qtrees/{volume.uuid}/{id}` に `name` |
| 名前マッピングの評価順（index）の変更 | 削除して作り直す | `PATCH` に `new_index` |
| クォータの有効・無効（ボリューム単位） | ONTAP CLI | `PATCH /storage/volumes/{uuid}` の `quota.enabled` |

> **Qtree の名前変更と index の移動を分けている理由**: どちらも「変更」ではなく「移動」です。
> Qtree の名前はクライアントがマウントしているパスの一部であり、name-mapping の index は
> 評価順序そのものです。上限や security style の変更と同じ操作として並べると、
> 影響範囲が同じだと読めてしまいます。
...
無く、空の一覧が返るだけです。選択はページを再読み込みすると既定（接続先の SVM）に戻ります。
