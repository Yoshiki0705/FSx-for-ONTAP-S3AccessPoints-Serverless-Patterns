...
|--------|:---:|:---:|:---:|:---:|
| ボリューム | ○ | ○ | ○ リサイズ | ○ |
| Qtree | ○ | ○ | ○ security style・エクスポートポリシー、名前変更 | ○ |
| クォータ | ○ | ○ | ○ 上限の変更、ボリューム単位の適用の開始・停止 | ○ |
| ストレージ効率 | ○ | — | — | — |
| FlexCache | ○ | ○ | ○ write-back の有効・無効、サイズ変更 | ○ |
| FlexClone | ○ | ○ | ○ 分割 | — |
...
| SMB 共有 | ○ | ○ | ○ | ○ |
| QoS ポリシー | ○ | ○ | ○ | ○ |
| ローカルユーザー | ○ | ○ ユーザー/グループ | ○ パスワード変更・有効/無効、メンバー追加・削除 | ○ ユーザー/グループ |
| 名前マッピング | ○ | ○ | ○ パターン・置換文字列、評価順（index）の移動 | ○ |
...
| スナップショット管理 | ○ | ○ ポリシー | ○ ロック有効化 | ○ ポリシー |
| SnapLock | ○ | — | ○ 保持期間 | — |
| SnapMirror | ○ | ○ 関係（宛先ボリュームの自動作成 + 初期化） | ○ 即時更新・一時停止・再開・break・resync・転送中止 | ○ 関係の削除 |
...
| パネル | 参照 | 作成 | 変更 | 削除 |
|--------|:---:|:---:|:---:|:---:|
| ピアリング | ○ クラスターピア/SVM ピア/intercluster LIF | ○ クラスターピア/SVM ピア | ○ 承認（passphrase / state）、SVM ピアの用途変更 | ○ クラスターピア/SVM ピア |
...
上の表の `—` のうち、「ONTAP 側に手段があるのにポータルから到達できない」ものは、現時点で
ありません。`—` として残っているのは ONTAP 自体が提供していない操作（SnapLock 保持期間の
短縮、FlexClone の削除など）です。

以前ここに挙げていた 4 件（FlexCache のサイズ変更、Qtree の名前変更、名前マッピングの
評価順の移動、ボリューム単位のクォータ適用）はいずれも実装し、検証環境の FSx for ONTAP
（`9.18.1P3D1`）で UI から実行して確認しました。

> **Qtree の名前変更と index の移動を別のボタンにしている理由**: どちらも「変更」ではなく
> 「移動」です。Qtree の名前はクライアントがマウントしているパスの一部であり、name-mapping の
> index は評価順序そのものです。上限や security style の変更と同じボタンの後ろに置くと、
> 影響範囲が同じだと読めてしまいます。名前変更は `confirm` を必須にし、どちらも操作の前に
> 何が動くかをその場に表示します。

> **クォータは「ルール」と「適用」が別**: ボリュームに上限を作っても、そのボリュームで
> 適用（enforcement）が停止していれば効きません。以前はこの状態が UI から見えず、確認するには
> ONTAP CLI が必要でした。クォータパネルはルールの表の上に現在の状態（適用中 / 停止中 /
> 初期化中）と切り替えを出します。ルールが 1 件も無いボリュームでは ONTAP が開始を拒否します。

### 対象 SVM の切り替え

ファイルシステムが複数の SVM を持つ場合、`リソース管理` のヘッダーに `対象 SVM` セレクターが
出ます（SVM が 1 つのときは選ぶものが無いので出ません）。選択した SVM は `svm` を読む
アクション（実測 66 個）に自動で渡され、切り替え時に管理系のクエリをすべて再取得します。

どのアクションが `svm` を受けるかはハンドラから生成しています。手で並べた一覧ではないので、
`svm` を読まないアクションに送って「未読パラメータ」で落ちることはありません。

FSx が内部で持つ `-fsx-mpu-svm` 接尾辞の SVM は選択肢から除いています。選んでも管理対象が
無く、空の一覧が返るだけです。

選択はタブのセッションのあいだ保たれます（再読み込みしても選び直す必要はありません）。
保持先は `sessionStorage` で、タブを閉じると消えます。`localStorage` にしていないのは、
数日前に選んだ SVM が次に開いたときの操作対象として残るのを避けるためです。
