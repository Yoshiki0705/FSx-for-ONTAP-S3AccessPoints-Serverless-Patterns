...
🌐 **Language / 言語**: [日本語](TROUBLESHOOTING-APPSYNC-AUTH.md) | English
...
## RETRACTED: `/storage/flexcache/flexcaches` was thought to reject `fsxadmin` with 401

**This conclusion was wrong and is kept only because the reasoning is instructive.** The 401 came from an `fsxadmin` password mismatch and lockout, not from any API restriction. The corrected analysis follows.
...
1. **Password mismatch**: The password in Secrets Manager did not match the actual `fsxadmin` password on the Amazon FSx for NetApp ONTAP filesystem
...
- SnapMirror list: ✅ `1 replication relationship` (svm_shift:ds_migtoaws → fsxsvm01:ds_migtoaws_bk)
