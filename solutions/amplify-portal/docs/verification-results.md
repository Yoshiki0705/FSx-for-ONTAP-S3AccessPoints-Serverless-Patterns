...
| 分類 | 対象 | 判断 |
|------|------|------|
| **A. 安全に実行できる** | **A6（SnapMirror の即時更新と転送中止）以外は 2026-08-15 に実施済み**。A6 だけが残る | A6 は自分のものでない関係にしか実行できないため保留。転送中止はソースにまとまったデータを書く手段が無く、この環境では検証できない。5 GiB 超のコピーも同じ理由で前提を作れない |
