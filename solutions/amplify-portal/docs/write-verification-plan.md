...
**A8 / A3 / A2 / A5 / A7 / A4 / A1 は実施済み（2026-08-15）。残るのは A6 のみ**で、これは自分の
ものでない SnapMirror 関係にしか実行できないため保留しています（下記）。
...
### A6. SnapMirror の即時更新と転送の中止 — **未実行（2026-08-15。自分のものでない関係にしか実行できないため保留）**
...
他クラスタ向けです。`fsxsvm01` ↔ `fsxsvm02` のクラスタ内ピアは存在しないため、自分の関係を作るには
**共有設定である SVM ピアを追加する**必要があります。既存の関係に `updateSnapmirrorNow` を打つと、
自分が所有していない宛先ボリュームに新しいスナップショットが作られ、転送履歴（追記のみ）も変わります。
どちらも共有環境に手を入れる操作なので、実行はアカウント所有者の判断を待ちます。

**中止（`abortSnapmirrorTransfer`）はこの環境では検証できません。** 中止するには続く長さの転送が必要で、
そのためにはソースボリュームにまとまったデータを書く手段（NFS / SMB クライアント）が要ります。
