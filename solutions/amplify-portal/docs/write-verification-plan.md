...
### A2. SMB 共有の作成・削除 — **実施済み（2026-08-15）**
...
### A3. ローカルグループとメンバー — **実施済み（2026-08-15）**
...
### A4. FlexClone の作成と分割（分割は不可逆） — **実施済み（2026-08-15）**
...
実測: `createFlexClone` は `clone.is_flexclone: true` を送っておらず、ONTAP は通常のボリューム作成と
読んで 787140 を返していました。**アグリゲートを足して 787140 を消すと成功が返りますが、できるのは
20 MB の普通のボリュームです**（クローン関係なし）。修正後は 20 GiB のクローンが親の
`clone_<name>.<timestamp>` を基点に作られます。分割はほぼ空のクローンで数秒、分割後の使用量は 348 KB
で容量は倍になりません。分割完了と同時にクローン一覧から消えるため進捗表示は実行中しか見えず、親側の
ベーススナップショットは残ります。

### A5. クォータルールの削除 — 削除しても効き続ける — **実施済み（2026-08-15）**
...
実測: Qtree 向け tree ルールを作ると ONTAP が既定の tree ルールも自動作成し、Qtree 側だけ削除すると
既定ルールが残って使用状況レポートにその Qtree が出続けます（削除した規則が残っているように見える形）。
削除された規則の上限自体はレポートから即座に消えました。適用が続くかどうかはこの読み取りでは観測
できないため、上のリファレンスの記述を根拠として扱い、ポータルは削除後に適用の off → on を案内します。
...
| 項目 | 内容 |
|------|------|
| 対象 | `updateArpStateAdmin`、`enableArpBulk`、`clearArpSuspects` |
| 前提 | ONTAP 9.10.1 以降（この環境は 9.18.1P3D1） |
| 影響 | **`dry_run` は選べません**。要求すると `enabled`（能動的な保護）になります。疑い検知でスナップショットが自動作成されます |
| 確認 | 応答が返す `state`（要求ではなく読み直した値）と、UI の表示が一致すること |
| 戻し方 | `disabled` に戻す。**ただし `disable_in_progress` のまま 10 分以上かかります** |

この項目は「dry_run は観測だけなので安全」という前提で A の先頭に置いていました。実測では
**ARP/AI に学習期間が存在せず、`dry_run` の要求は無言で `enabled` になります**（エラーも
警告もありません）。詳細は [ARP/AI と EMS の罠](../../../docs/agent/pitfalls-arp-ems.md)。

したがってこの操作は「観測モードを試す」ではなく「保護を有効にする」です。実施するかは
そのボリュームで自動スナップショットが作られてよいかの判断になります。検証は throwaway
ボリュームで行い、終わったらボリュームごと削除するのが最も確実です（無効化に時間がかかり、
削除すれば ARP 設定も一緒に消えます）。
...
> **この項目から得た一般的な教訓**: 「安全な観測モードがある」は前提であって事実ではありません。
> 計画段階でドキュメントを読んでいても、そのモードが**現行バージョンに存在するか**は実測でしか
> 分かりませんでした。安全側の前提こそ先に確かめる価値があります。
