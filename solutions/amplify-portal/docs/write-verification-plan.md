...
**A1 から A8 まですべて実施済み（2026-08-15）**。残る未実行は A6 の**転送の中止**と、A7 の
**5 GiB を超えるコピー**の 2 点だけで、どちらも理由が別にあります（それぞれの節に記載）。

実行順は A8 → A3 → A2 → A5 → A7 → A4 → A1 でした。前提が少なく、影響が小さく、戻しやすいもの
から並べています。実施の結果、A1・A5・A8 は**当初の前提が実測で覆りました**。

### A1. QoS ポリシー — **実施済み（2026-08-15）。「一巡が完了しない」という当初の前提は実測で否定**
...
実測（9.18.1P3D1 / REST）: **割り当て中のポリシーも削除できます**。CLI リファレンスの記述と異なり
`force` は要らず、削除と同時にボリューム側の割り当ても外れて上限が 0（無制限）になります。したがって
「一巡が完了しない」という前提は成り立ちませんでした。代わりに、**削除はそのポリシーを使っている全
ボリュームの上限を外す操作**でもあるため、パネルの確認文でそれを述べるようにしました。`none` による
解除は、ポリシーを残したまま 1 つのボリュームの上限だけを外す手段として引き続き必要です。
...
### A5. クォータルールの削除 — **実施済み（2026-08-15）。「削除しても効き続ける」はレポート上は観測できず**
...
### A6. SnapMirror の即時更新と転送の中止 — **即時更新は実施済み（2026-08-15、承認のうえ既存関係に実行）。中止は未実行**
...
この環境の SnapMirror 関係は 2 件とも他システム由来で（ソースが外部クラスタ）、SVM ピアもすべて
他クラスタ向けです。`fsxsvm01` ↔ `fsxsvm02` のクラスタ内ピアが無いため自分の関係は作れず、**アカウント
所有者の承認を得て既存関係に `updateSnapmirrorNow` を実行**しました（宛先に新しいスナップショットが
作られ、転送履歴が 1 件増えます。どちらも元に戻せない種類の変化です）。

実測: 転送は **12 秒 / 27,888 バイト**で完了し、`transferring` の状態を観測できました。ラグは
14h59m → 1m8s、`lastTransferType` は resync → update、関係は snapmirrored / healthy を維持。

**中止（`abortSnapmirrorTransfer`）は実行していません。** 当初「転送が一瞬で終わるので中止できない」と
書いていましたが、上の実測どおり 12 秒あるので**技術的には狙えます**（update を打ってから 1 秒間隔で
`transferring` を待ち、現れたら中止する）。実行しない理由は別で、**中止は自分が所有していない関係を
unhealthy にし、再開用チェックポイントを残す**ためです。実行するならこれも所有者の承認が必要です。
...
クローンを削除したあとに親を削除しようとすると `has one or more clones` で止まります。原因は
ONTAP の volume recovery queue（既定 12 時間）で、`purge` は diag 権限が必要なため FSx では
待つしかありません。**クローンを分割してから削除すれば親はすぐ削除できる**ことを A/B で確認済み
なので、親を消す予定があるなら分割を選択肢に入れます。分割の性質と使いどころは FlexClone
パネルの折りたたみに載せました。
