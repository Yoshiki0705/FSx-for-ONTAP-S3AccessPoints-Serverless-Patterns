...
> 画面からは `SnaplockConfirmDialog` が前段に立ち、**キーワードの入力またはチェックボックス**を
> 要求します。ハンドラ側の `acknowledgeIrreversible` はフロントエンドがリテラルで送りますが、
> これはスクリプトやエージェントからの直接呼び出しを止めるためのもので、二重のゲートです。
> **止められないのは、確認画面を読んだ人が納得して進める判断**です。
>
