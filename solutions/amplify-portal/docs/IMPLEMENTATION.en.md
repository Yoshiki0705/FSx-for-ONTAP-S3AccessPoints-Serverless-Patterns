...
| cdk-nag `AwsSolutionsChecks` | Blocking in CI (Stage 2b, against `security/cdk-nag-baseline.txt`); off in the deployment path, enabled by `CDK_NAG=1` — registered via `Validations.of(root).addPlugins(...)`. NOT applied during synth/deploy: Amplify Gen2 resources (AppSync, Cognito, internal S3) produce findings that are not user-configurable, and a reported violation interrupts synthesis and blocks every deploy. Accepted findings are listed with `Validations.of(dataStack).acknowledge(...)` in backend.ts, but measured 2026-08-27 those coarse entries suppress nothing; the 105 known findings are held in `security/cdk-nag-baseline.txt` instead. cdk-nag v3 removed `NagSuppressions` and moved off `IAspect`. |
...
| 2026-08-12 | File-list thumbnails | Backend generation with an ETag-keyed cache. One batched call per page rather than a presigned URL per row. Pillow layer | functions/thumbnails/, useThumbnails.ts, FilePreview.tsx |
...
| [Security Review Template](../../../docs/en/security-review-template.md) | Threat model, IAM permissions, data flow analysis |
