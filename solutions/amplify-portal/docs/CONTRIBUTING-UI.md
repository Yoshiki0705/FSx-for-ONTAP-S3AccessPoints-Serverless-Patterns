...
**いきなり新機能を作らないでください。** 下の 3 フェーズは、触る範囲とゲートの数がこの順に増えます。
フェーズ 1 を通せば「変更が画面に出るまでの一周」が分かり、フェーズ 3 で「新しい ONTAP 操作を足す」
一周が分かります。

### フェーズ 1（10 分）— 表示されている文字を 1 つ変える
...
2. 表のヘッダ（`<th>`）に `t("...")` で見出しを足す。文字列はフェーズ 1 の手順で `ja.ts` へ
...
### VolumeSelector の `onSelect` が渡す `null`
