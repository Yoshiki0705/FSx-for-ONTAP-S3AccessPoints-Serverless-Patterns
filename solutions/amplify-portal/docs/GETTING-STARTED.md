...
本プロジェクトは **FSx for ONTAP（NAS）上にデータを持っているチーム** が対象です。

| あなたの環境 | このポータルの使い方 |
|-----------|-------------------|
| NAS のみ（SaaS 未使用） | ブラウザアクセス・AI 処理・共有リンク・監査証跡まで本ポータルで完結 |
| NAS + Box/SharePoint/Google Drive 併用 | SaaS はそのまま。NAS 上の大容量データへの AI 処理・監査・データ保護を追加 |
