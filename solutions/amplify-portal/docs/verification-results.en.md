...
| **An SVM with an ONTAP S3 server cannot host one** | On fsxsvm01 it went `FAILED` with `existing ONTAP object storage server on SVM ...`. **A native ONTAP S3 server and an FSx for ONTAP S3 AP do not coexist on one SVM** |
