...
| Splitting before deleting a clone frees the parent at once (measured A/B) | The same environment, the same steps, only the split differing. Delete an unsplit clone → the parent's delete fails (`has one or more clones`, still after 7 and 15 minutes). Split → delete → the parent deletes seconds later. The cause is ONTAP's volume recovery queue: a deleted RW/DP volume is held for 12 hours by default, keeps consuming aggregate space, and still counts as a clone from the parent's side. `purge` needs diag privilege, which FSx's fsxadmin does not have | [Volume Recovery Queue (KB)](https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/How_to_use_the_Volume_Recovery_Queue) |
...
| Starting a rebalance | **Not verified.** Starting enables granular data, which **cannot be disabled** (the only way back is restoring a snapshot taken before it was enabled). Irreversible, so it is waiting on approval | Same |
