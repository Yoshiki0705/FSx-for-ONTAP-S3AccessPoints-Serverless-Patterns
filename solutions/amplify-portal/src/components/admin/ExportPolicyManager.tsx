...
export function ExportPolicyManager() {
  ...
  const {
    ...
    queryFn: () =>
      unwrap<{ policies?: ExportPolicy[] }>(
        dispatch("adminQuery", { action: "listExportPolicies" }),
      ).then((d) => d?.policies ?? []),
  });
  ...
  const loadRules = async (policyId: string) => {
    try {
      const data = await adminQuery<{ rules?: ExportRule[] }>({
        action: "getExportPolicyRules",
        params: { policyId },
      });
      if (data) {
  ...
  const handleCreatePolicy = async () => {
    ...
    try {
      const data = await adminMutate<{ success?: boolean }>({
        ...
        params: { name: newPolicyName.trim() },
      });
      if (data) {
        if (data.success) {
  ...
  const handleDeletePolicy = async (policy: ExportPolicy) => {
    ...
    try {
      const data = await adminMutate<{ success?: boolean }>({
        ...
        params: { policyId: String(policy.id), confirm: true },
      });
      if (data) {
        if (data.success) {
  ...
  const handleDeleteRule = async (ruleIndex: number) => {
    ...
    try {
      const data = await adminMutate<{ success?: boolean }>({
        action: "deleteExportPolicyRule",
        params: { policyId: String(selectedPolicy.id), ruleIndex },
      });
      if (data) {
