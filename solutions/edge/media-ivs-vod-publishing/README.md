...
| **非サポート（Not supported・AWS 確認済み）** | `Confirmed unsupported` | IVS Recording Configuration の出力先に FSx for ONTAP S3 Access Point alias を直接指定する構成。**AWS サービスチームが非サポートと確認済み**（サポートされる出力先は標準 Amazon S3 バケット）。alias は `bucketName` が bucket 名形式（63 文字以内）として検証されるだけのため config 作成は `ACTIVE` になるが、録画時の書き込みは失敗する（検証環境で **「Recording Start Failure」**、`ivs/v1/...` は未書き込み。IVS SLR に AP ポリシーで許可しても同様）。標準 S3 バケット経由の推奨パスを使用。[direct-recording-experiment.md](direct-recording-experiment.md) を参照。 |

> 本パターンは **リファレンス実装** です。配信ベンダーの選定、権利処理、地域制限、コンプライアンスは
> 利用者・組織が判断します。技術的検証は法務・コンプライアンス・プライバシー評価を代替しません。
...
> VOD 再配信する。直接録画（IVS→FSx for ONTAP S3 AP）は **AWS が非サポートと確認済み**（標準 S3 バケット経由が唯一のサポート経路）。
...
- **ライブ並走の near-live 共同編集**にも広がる（配信中に追っかけ編集・字幕を扱う）。ただし IVS ライブ
  マニフェストへの直接差し込みは不可で、実現レイヤーを分けて設計する（[architecture.md](architecture.md) の
  「ライブ並走の near-live 共同編集」参照）。
...
- **最初に確認する問い**: 「ライブ配信後の編集・QC・承認・アーカイブに、ファイル（NFS/SMB）と S3 API の
...
| Go/No-Go | 直接録画（IVS→FSx for ONTAP S3 AP） | **非サポート**（AWS サービスチーム確認済み）。標準 S3 バケット経由を使用 |
...
- Direct 録画は **非サポート**（AWS サービスチーム確認済み。[direct-recording-experiment.md](direct-recording-experiment.md)）。
