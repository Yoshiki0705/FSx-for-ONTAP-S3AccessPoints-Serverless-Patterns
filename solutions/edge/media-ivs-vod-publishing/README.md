...
| **実験（Experimental）** | `Observed: recording-time failure` | IVS Recording Configuration の出力先に FSx for ONTAP S3 Access Point alias を直接指定する構成。**AWS 公式ドキュメント上でサポートと明記されていない**。検証環境では、設定作成は `ACTIVE` になるが、実ライブ配信では **「Recording Start Failure」** が発生し `ivs/v1/...` は AP に書かれなかった（IVS SLR に AP ポリシーで許可しても同様）。実運用は推奨パスを使用。[direct-recording-experiment.md](direct-recording-experiment.md) を参照。 |
...
- **「IVS の録画先を直接 FSx for ONTAP S3 AP にできる？」** 設定作成は `ACTIVE` になるが、検証環境の実ライブ配信では
  **「Recording Start Failure」** が発生し `ivs/v1/...` は書かれなかった。公式サポート明記もないため Experimental として扱う
