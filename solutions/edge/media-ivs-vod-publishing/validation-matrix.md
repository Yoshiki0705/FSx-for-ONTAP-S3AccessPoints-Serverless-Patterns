...
| 2 | IVS RecordingConfiguration with **FSx for ONTAP S3 AP alias** as `bucketName` | **Experimental / not supported** (observed recording-time failure) | See [direct-recording-experiment.md](direct-recording-experiment.md): config creation reaches ACTIVE but recording-time write-through was observed to fail | High — recording-time write-through **observed to fail** | [CreateRecordingConfiguration API](https://docs.aws.amazon.com/ivs/latest/LowLatencyAPIReference/API_CreateRecordingConfiguration.html) accepts `destinationConfiguration.s3.bucketName`; [S3 AP alias usage](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points-alias.html) | **Observed (test env)**: config with the AP **alias** reached `ACTIVE` (standard S3 AP and FSx for ONTAP S3 AP); the **ARN** was rejected (`bucketName` max length 63). **Recording-time**: a live stream to an FSx for ONTAP S3 AP alias produced **`Recording Start Failure`** and wrote **no `ivs/v1/...` objects** (even with the IVS SLR granted on the AP; empty reason code) → **Experimental, not Supported; use IVS → standard S3 → FSx for ONTAP** |
