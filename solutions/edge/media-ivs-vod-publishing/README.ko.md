...
| **실험(Experimental)** | `Observed: recording-time failure` | IVS Recording Configuration 의 출력 대상으로 FSx for ONTAP S3 Access Point alias 를 직접 지정하는 구성. **AWS 공식 문서에서 지원으로 명시되지 않음**. 검증 환경에서 설정 생성은 `ACTIVE` 가 되었으나, 실제 라이브 스트림에서는 **"Recording Start Failure"** 가 발생하고 `ivs/v1/...` 객체가 AP 에 기록되지 않음(AP 정책으로 IVS SLR 에 허용해도 동일). 실운영은 권장 경로 사용. [direct-recording-experiment.md](direct-recording-experiment.md) 참조. |
...
- **"IVS 녹화를 FSx for ONTAP S3 AP 로 직접?"** 설정 생성은 `ACTIVE` 가 되지만, 검증 환경의 실제 라이브 스트림에서는
  **"Recording Start Failure"** 가 발생하고 `ivs/v1/...` 객체가 기록되지 않음. 공식 지원 명시도 없어 Experimental 로 취급
