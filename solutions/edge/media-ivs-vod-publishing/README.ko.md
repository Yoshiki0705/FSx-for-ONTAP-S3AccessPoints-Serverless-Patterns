...
| **비지원(Not supported·AWS 확인)** | `Confirmed unsupported` | IVS Recording Configuration 의 출력 대상으로 FSx for ONTAP S3 Access Point alias 를 직접 지정하는 구성. **AWS 서비스 팀이 비지원으로 확인**(지원되는 출력 대상은 표준 Amazon S3 버킷). alias 는 `bucketName` 이 버킷 이름 형식(63자 이내)으로만 검증되어 config 생성은 `ACTIVE` 가 되지만, 녹화 시 쓰기는 실패함(검증 환경에서 **"Recording Start Failure"**, `ivs/v1/...` 미기록. AP 정책으로 IVS SLR 에 허용해도 동일). 표준 S3 버킷 경유 권장 경로 사용. [direct-recording-experiment.md](direct-recording-experiment.md) 참조. |
...
> 직접 녹화(IVS→FSx for ONTAP S3 AP)는 **AWS 가 비지원으로 확인**(표준 S3 버킷 경유가 유일한 지원 경로).
...
| Go/No-Go | 직접 녹화(IVS→FSx for ONTAP S3 AP) | **비지원**(AWS 서비스 팀 확인). 표준 S3 버킷 경유 사용 |
...
| IVS RecordingConfiguration + FSx for ONTAP S3 AP alias | Not supported(AWS 확인) |
...
- 직접 녹화는 **비지원**(AWS 서비스 팀 확인. [direct-recording-experiment.md](direct-recording-experiment.md)).
...
- [설계](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/en/playbooks/02-design) — 이 패턴에서 판단이 집중되는 곳
