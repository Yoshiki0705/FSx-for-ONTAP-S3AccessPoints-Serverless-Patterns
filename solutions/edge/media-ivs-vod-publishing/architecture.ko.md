...
> **Partner/SI**(delivery lens): 페이즈 1–2 는 30 분·FSx 불필요 PoC 로 첫 고객 대화에 적합. 페이즈 3–5 는
> 고객 실환경에 매핑되며 사이징과 거버넌스 승인이 이뤄지는 지점.
...
- **"IVS 녹화를 FSx for ONTAP S3 Access Point 로 직접?"** 공식 지원 명시 없음 — Experimental 로 다루고 검증
