...
## ライブ並走の near-live 共同編集（3 レイヤー整理）

「配信中に、FSx for ONTAP から S3 Access Point 経由で追っかけ編集や字幕を挿入したい」という要望は
自然だが、IVS のライブ配信の仕組み上、**どのレイヤーで挿入するか**を分けて設計する必要がある。

| レイヤー | 何ができるか | FSx for ONTAP S3 AP の関与 |
|---------|-------------|---------------------------|
| **1. IVS ライブ配信パス（IVS マネージド）** | encoder → IVS トランスコード/パッケージング → IVS の CDN で視聴者へ。この**ライブ再生マニフェストは IVS が管理**し、外部で用意した HLS セグメント/字幕を後から差し込む口はない。 | **不可**（ライブマニフェストは不可変）。サーバー側でライブを加工する要件は AWS Elemental MediaLive / MediaPackage の領域。 |
...
- **真のライブではなく near-live**: セグメント確定 → 取り込み → 編集 → 再配信の分だけ必ず遅延する。
  「追っかけ」は数十秒〜分オーダーの遅れが前提。
- **IVS ライブマニフェストは不可変**: レイヤー 1 への差し込みは不可。
- **字幕のライブ焼き込み**はエンコーダー側（IVS の外）。
- レイヤー 2 は `PutMetadata` の 1 KB / 5 TPS 上限に収まるよう、実体ではなく参照を載せる設計にする。

> **利用者価値**（Partner/SI lens）: このパターンの訴求は「ライブ後の VOD 化」だけでなく、
...
- **「IVS の録画先を直接 FSx for ONTAP の S3 Access Point にできる？」** 設定作成は `ACTIVE` になるが、検証環境の
  実ライブ配信では **「Recording Start Failure」** が発生し `ivs/v1/...` は書かれなかった。公式サポート明記もなく Experimental
  として扱う（[direct-recording-experiment.md](direct-recording-experiment.md)）。
