...
- 工場のファイルサーバーに蓄積される CSV センサーログを定期的に分析したい
- 品質検査画像の目視確認を AI で自動化・効率化したい
- 既存の NAS ベースのデータ収集フロー（PLC → ファイルサーバー）を変更せずに分析を追加したい
- Athena SQL による柔軟な閾値ベース異常検出を実現したい
- Rekognition の信頼度スコアに基づく段階的な判定（自動合格 / 手動レビュー / 自動不合格）が必要

### このパターンが適さないケース

- ミリ秒単位のリアルタイム異常検出が必要（IoT Core + Kinesis を推奨）
- TB 規模のセンサーログを一括処理（EMR Serverless Spark を推奨）
- 画像の欠陥検出に独自の学習済みモデルが必要（SageMaker エンドポイントを推奨）
- センサーデータが既に時系列 DB（Timestream 等）に格納されている
...
### Outcome
IoT センサーログ・品質検査画像の自動分析により、異常検知の迅速化と品質管理工数を削減する。

### Metrics
| メトリクス | 目標値（例） |
|-----------|------------|
| 分析対象ファイル数 / 実行あたり | > 1,000 files |
| 異常検知レイテンシ | < 1 時間（POLLING） |
| 誤検知率 (False Positive) | < 5% |
| 処理スループット | > 500 files/hour |
| コスト / スキャン | < $5 |
| Human Review 対象率 | < 5%（アラート通知のみ） |

### Measurement Method
CloudWatch Metrics (FilesProcessed, AnomaliesDetected)、Athena クエリ結果、SNS 通知ログ。
...
- [評価](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/ja/playbooks/01-assess) — 最初に読む
- [データ活用](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/ja/domains/data-utilization) — 次に読む
