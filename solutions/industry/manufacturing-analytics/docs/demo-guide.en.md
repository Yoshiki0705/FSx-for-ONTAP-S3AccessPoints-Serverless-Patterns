...
**Design**: Sensor data analysis results, anomaly detection reports, and image inspection results are all written back via FSx for ONTAP S3 Access Point to the **same FSx for ONTAP volume** as the original sensor CSV and inspection images. No standard S3 bucket is created ("no data movement" pattern).
