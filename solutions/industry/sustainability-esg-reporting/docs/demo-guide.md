# サステナビリティ・ESG — メトリクス抽出 / フレームワークマッピング Demo Guide
...
| 項目 | 詳細 |
|------|------|
| **役職** | CSR/ESG 推進部長 / サステナビリティ報告責任者 |
| **日常業務** | ESG データ収集、開示レポート作成、規制対応 |
| **課題** | 散在する ESG メトリクスの手動集約、開示基準の頻繁な変更対応 |
| **期待する成果** | ESG データの自動抽出とレポーティング効率化 |
...
cd fsxn-s3ap-serverless-patterns/solutions/industry/sustainability-esg-reporting
```
...
sam deploy \
  --stack-name fsxn-esg-reporting-demo \
  --parameter-overrides \
...
aws cloudformation delete-stack --stack-name fsxn-esg-reporting-demo --region ap-northeast-1
aws cloudformation wait stack-delete-complete --stack-name fsxn-esg-reporting-demo --region ap-northeast-1
```
...
```bash
# Lambda パッケージ作成
cd solutions/industry/sustainability-esg-reporting
sam build
