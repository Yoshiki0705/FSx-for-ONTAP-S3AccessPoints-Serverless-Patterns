...
FSx for ONTAP S3 Access Points를 활용한 위성 이미지(GeoTIFF / NITF / HDF5)의
...
```mermaid
graph LR
    FSx[FSx for ONTAP<br/>위성 이미지 스토리지] --> S3AP[S3 Access Point<br/>NTFS ACL 연동]
...
- FSx for ONTAP는 위 모든 Impact Level에서 승인됨
...
- FSx for ONTAP, S3 Access Points, Lambda, Step Functions 모두 커버
...
| `FSXN_S3AP` | FSx for ONTAP S3 Access Point | 없음(기존 FSx 볼륨에 재기록) | 분석 담당자가 SMB/NFS 경유로 원본 위성 이미지와 동일 디렉터리에서 AI 산출물을 열람 |
