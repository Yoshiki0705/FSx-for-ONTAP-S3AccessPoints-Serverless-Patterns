...
利用 FSx for ONTAP S3 Access Points 实现卫星图像（GeoTIFF / NITF / HDF5）的
...
```mermaid
graph LR
    FSx[FSx for ONTAP<br/>卫星图像存储] --> S3AP[S3 Access Point<br/>NTFS ACL 联动]
...
- FSx for ONTAP 已获上述所有 Impact Level 批准
...
- FSx for ONTAP、S3 Access Points、Lambda、Step Functions 全部覆盖
...
| `FSXN_S3AP` | FSx for ONTAP S3 Access Point | 无（写回现有 FSx 卷） | 分析人员通过 SMB/NFS 在与原始卫星图像相同的目录中查看 AI 成果物 |
