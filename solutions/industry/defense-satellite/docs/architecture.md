...
FSx for ONTAP S3 Access Points を活用した衛星画像（GeoTIFF / NITF / HDF5）の
...
```mermaid
graph LR
    FSx[FSx for ONTAP<br/>衛星画像ストレージ] --> S3AP[S3 Access Point<br/>NTFS ACL 連動]
...
- FSx for ONTAP は上記すべての Impact Level で承認済み
...
- FSx for ONTAP、S3 Access Points、Lambda、Step Functions すべてカバー
...
UC15 は 2026-05-11 のアップデートで `OutputDestination` パラメータをサポートしました。

| モード | 出力先 | 作成されるリソース | ユースケース |
|-------|-------|-------------------|------------|
| `STANDARD_S3`（デフォルト） | 新規 S3 バケット | `AWS::S3::Bucket` | 従来どおり分離された S3 バケットに AI 成果物を蓄積 |
| `FSXN_S3AP` | FSx for ONTAP S3 Access Point | なし（既存 FSx ボリュームへ書き戻し） | 分析担当者が SMB/NFS 経由でオリジナル衛星画像と同一ディレクトリに AI 成果物を閲覧 |
