...
利用 FSx for ONTAP S3 Access Points 的衛星影像（GeoTIFF / NITF / HDF5）
...
```mermaid
graph LR
    FSx[FSx for ONTAP<br/>衛星影像儲存] --> S3AP[S3 Access Point<br/>NTFS ACL 連動]
...
- FSx for ONTAP 已獲上述所有 Impact Level 核准
...
- FSx for ONTAP、S3 Access Points、Lambda、Step Functions 全部涵蓋
...
| `FSXN_S3AP` | FSx for ONTAP S3 Access Point | 無（寫回既有 FSx 磁碟區） | 分析人員透過 SMB/NFS 在與原始衛星影像相同目錄中檢視 AI 成果物 |
