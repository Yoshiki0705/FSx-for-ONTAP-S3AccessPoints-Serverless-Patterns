...
本示範展示了合約書與請求書的自動處理管線。結合 OCR 文字擷取與實體擷取，從非結構化文件自動生成結構化資料。

**示範核心訊息**：將紙本合約書與請求書自動數位化，即時擷取並結構化金額、日期、交易對象等重要資訊。
...
**旁白要旨**:
> 將擷取結果輸出為結構化資料。金額、付款期限、交易對象名稱、請求書編號等以 JSON 格式提供使用。
...
**旁白要旨**:
> AI 評估擷取結果的信賴度，標記低信賴度項目。透過處理摘要報告掌握整體處理狀況。
...
寫回至與原始請求書 PDF **相同的 FSx for ONTAP 磁碟區**。不會建立標準 S3 儲存貯體
（"no data movement" 模式）。
...
與 Phase 7 UC15/16/17 及 UC6/11/14 的示範採用相同方針，以**終端使用者在日常業務中實際
