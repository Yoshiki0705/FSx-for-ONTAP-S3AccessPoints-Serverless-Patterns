...
### Outcome
契約書・請求書の自動処理により、手動データ入力工数を削減する。

### Metrics
| メトリクス | 目標値（例） |
|-----------|------------|
| 処理済み文書数 / 実行あたり | > 500 documents |
| OCR 精度（文字認識率） | > 95% |
| データ抽出成功率 | > 90% |
| 処理時間 / 文書 | < 30 秒 |
| コスト / 文書 | < $0.10 |
| Human Review 対象率 | < 20%（低信頼度スコア） |

### Measurement Method
Step Functions 実行履歴、Textract confidence score、CloudWatch Metrics、S3 出力ファイル数。
...
- [ブロックストレージ](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/tree/main/docs/ja/domains/block-storage) — このパターンで判断が集中するところ
