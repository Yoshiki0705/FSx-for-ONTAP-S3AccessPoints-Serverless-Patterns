...
### Outcome
ファイルサーバー監査・コンプライアンスチェックの自動化により、手動監査工数を削減する。

### Metrics
| メトリクス | 目標値（例） |
|-----------|------------|
| スキャン対象ファイル数 / 実行あたり | > 1,000 files |
| 過剰権限検出数 / スキャン | 可視化（ベースライン確立） |
| コンプライアンスレポート生成時間 | < 5 分 |
| 手動監査工数の削減率 | > 50% |
| スキャンあたりのコスト | < $1 |
| Human Review 対象率 | < 10%（高リスク検出のみ） |

### Measurement Method
Step Functions 実行履歴、CloudWatch Metrics (FilesProcessed, Duration)、生成レポートのメタデータ、SNS 通知ログ。
...
|------|-------------|-------------------|
| ファイル検出 | 数時間（手動棚卸し） | 36 ms (10 files) |
| ファイル読み取り | 個別アクセス | avg 37 ms / file |
| 全体処理時間 | 数時間〜数日 | 404 ms (10 files, sequential) |
| レポート形式 | 非標準化 | JSON メタデータ + 監査レポート |
| レビュー体制 | 担当者依存 | Human Review Queue |
| 監査証跡 | 個人記録 | DynamoDB + CloudWatch |

> **注記**: 上記は小規模サンプルランの結果であり、本番環境のスループット見積もりや性能保証ではありません。UC1 の sample run は合成または非機微のサンプルファイルを使用しており、導入先の法務文書を表すものではありません。本 sample run は処理パスの検証のみです。法的妥当性、分類品質、レビュー完全性は、導入先固有の PoC で別途評価してください。
