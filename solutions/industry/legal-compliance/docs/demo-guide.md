...
オリジナル契約データと**同一の FSx for ONTAP ボリューム**に書き戻されます。標準 S3 バケットは
...
## 実行時間の目安（Phase 8 検証実績）

UC1 の処理時間は ONTAP ボリューム上のファイル数に比例します。

| ステップ | 処理内容 | 実測値 (549 ファイル) |
|---------|---------|---------------------|
| Discovery | ONTAP REST API でファイル一覧取得 | 8 分 |
| AclCollection (Map) | 各ファイルの NTFS ACL 収集 | 2 時間 20 分 |
| AthenaAnalysis | Glue Data Catalog + Athena クエリ | 5 分 |
| ReportGeneration | Bedrock Nova Lite でレポート生成 | 5 分 |
| **合計** | | **2 時間 38 分** |
...
- **Map state MaxConcurrency**: デフォルト 40 → 100 に上げると AclCollection 時間を短縮可能
- **Lambda メモリ**: Discovery Lambda は 512MB 以上を推奨（VPC ENI アタッチ高速化）
- **Lambda タイムアウト**: 大量ファイル環境では 900s を推奨（デフォルト 300s では不足）
- **SnapStart**: Python 3.13 + SnapStart でコールドスタートを 50-80% 削減可能
