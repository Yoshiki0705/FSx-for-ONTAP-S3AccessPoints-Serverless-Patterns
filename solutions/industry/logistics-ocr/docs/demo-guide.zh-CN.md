...
本演示展示了配送单据的 OCR 处理和库存分析管道。将纸质单据数字化，自动汇总和分析入库出库数据。
...
**解说要点**:
> 每天 500 张以上的配送单据。手动输入导致库存信息更新延迟，缺货和库存过剩的风险增加。
...
**解说要点**:
> 只需将扫描的单据图像放置到文件夹中，OCR 管道就会自动启动。
...
**解说要点**:
> AI 生成库存分析报告。展示库存周转率、缺货风险品目、订货建议。

**Key Visual**: AI 生成库存报告（库存摘要 + 订货建议）
...
| Report | 3:45–5:00 | "AI 提示缺货风险和订货建议" |
...
SMB/NFS 用户可以在业务使用的目录结构中直接查看 AI 成果物。
不会创建标准 S3 存储桶。
...
1. **事前准备**：
   ...
   - 使用 `UC=logistics-ocr bash scripts/package_generic_uc.sh` 打包 Lambda
   - 使用 `bash scripts/deploy_generic_ucs.sh UC12` 部署

2. **放置样本数据**：
   - 通过 S3 AP Alias 将样本文件上传到 `waybills/` 前缀
   - 启动 Step Functions `fsxn-logistics-ocr-demo-workflow`（输入 `{}`）

3. **拍摄**（关闭 CloudShell·终端，浏览器右上角的用户名涂黑）：
   - S3 输出存储桶 `fsxn-logistics-ocr-demo-output-<account>` 的俯瞰图
   - AI/ML 输出 JSON 的预览（参考 `build/preview_*.html` 格式）
   - SNS 邮件通知（如适用）

4. **遮罩处理**：
   - 使用 `python3 scripts/mask_uc_demos.py logistics-ocr-demo` 自动遮罩
