...
FSx for ONTAP の S3 Access Points を活用し、FASTQ/BAM/VCF ゲノムデータの品質チェック、バリアントコール統計集計、研究サマリー生成を自動化するサーバーレスワークフローです。

### このパターンが適しているケース

- 次世代シーケンサーの出力データ（FASTQ/BAM/VCF）が FSx for ONTAP 上に蓄積されている
...
| サービス | 単価 | 想定使用量 | 月額概算 |
|---------|------|-----------|---------|
| Lambda | $0.0000166667/GB-sec | 5 関数 × 50 samples/日 | ~$1-5 |
