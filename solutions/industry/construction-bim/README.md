...
FSx for ONTAP の S3 Access Points を活用し、BIM モデル（IFC/Revit）のバージョン管理、図面 PDF の OCR テキスト抽出、安全コンプライアンスチェックを自動化するサーバーレスワークフローです。

### このパターンが適しているケース

- BIM モデル（IFC/Revit）や図面 PDF が FSx for ONTAP 上に蓄積されている
