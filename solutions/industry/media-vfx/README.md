...
- 品質合格時は S3 AP 経由で FSx for ONTAP に PutObject、不合格時は SNS 通知


## Success Metrics

### Outcome
VFX アセットの自動分類・メタデータ付与により、アセット検索時間を短縮する。

### Metrics
| メトリクス | 目標値（例） |
|-----------|------------|
| 処理済みアセット数 / 実行あたり | > 200 files |
| メタデータ付与成功率 | > 95% |
| アセット検索時間の短縮 | > 60% |
| 処理時間 / ファイル | < 60 秒 |
| コスト / 実行 | < $10 |
...
### Measurement Method
Step Functions 実行履歴、Rekognition label count、S3 出力メタデータ。
