...
本示範展示學術論文的自動分類與引用網絡分析管道。從大量論文 PDF 中提取元數據，並可視化研究趨勢。
...
**旁白要旨**：
> 收集了 500 篇以上的論文 PDF。希望掌握按領域的分布、重要論文、研究趨勢，但全部閱讀是不可能的。
...
**旁白要旨**：
> 分析引用關係，識別被引用次數多的重要論文。分析引用網絡的結構。
...
3. **拍攝**（關閉 CloudShell・終端機，將瀏覽器右上角的使用者名稱塗黑）：
   - S3 輸出儲存貯體 `fsxn-education-research-demo-output-<account>` 的俯瞰
