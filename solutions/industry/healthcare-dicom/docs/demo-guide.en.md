...
**Design**: DICOM metadata, de-identification results, and PII detection logs are all written back via FSx for ONTAP S3 Access Point to the **same FSx for ONTAP volume** as the original DICOM medical images. No standard S3 bucket is created ("no data movement" pattern).
