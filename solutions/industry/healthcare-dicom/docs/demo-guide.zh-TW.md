...
**示範的核心訊息**：從 DICOM 檔案自動移除患者識別資訊，安全地生成可用於研究的匿名化資料集。
...
| **課題** | 大量 DICOM 檔案的手動匿名化耗時且有錯誤風險 |
| **期待成果** | 安全可靠的匿名化與稽核軌跡自動化 |
...
**旁白要旨**：
> 為了多機構合作研究需要匿名化 10,000 個 DICOM 檔案。手動處理有錯誤風險，絕不允許個人資訊外洩。

**Key Visual**：DICOM 檔案清單、患者資訊標籤的醒目提示
...
**旁白要旨**：
> 自動處理各 DICOM 檔案的個人資訊標籤。患者姓名→雜湊、出生日期→年齡範圍、機構名稱→匿名代碼。保留影像像素資料。
...
| 章節 | 時間 | 關鍵訊息 |
|-----------|------|--------------|
| Problem | 0:00–0:45 | 「大量 DICOM 的匿名化遺漏絕不允許」 |
...
寫回至與原始 DICOM 醫用影像**相同的 FSx for ONTAP 磁碟區**。不會建立標準 S3 儲存貯體
（"no data movement" 模式）。
...
- 📸 **UI/UX 拍攝**：✅ SFN Graph 完成（Phase 8 Theme D, commit c66084f）
...
![UC5 Step Functions Graph（縮放顯示 — 各步驟詳細）](../../docs/screenshots/masked/uc5-demo/step-functions-graph-zoomed.png)

### 重新驗證時的 UI/UX 對象畫面（建議拍攝清單）

- S3 輸出儲存貯體（dicom-metadata/、deid-reports/、diagnoses/）
- Comprehend Medical 實體偵測結果（Cross-Region）
- DICOM 匿名化後的中繼資料 JSON
...
2. **範例資料配置**：
   - 透過 S3 AP Alias 將範例檔案上傳至 `dicom/` 前綴
