...
def test_classify_by_keywords_confidential(classification_handler):
    level, conf = classification_handler.classify_by_keywords("This document contains TOP SECRET information")
...
def test_classify_by_keywords_public(classification_handler):
    level, conf = classification_handler.classify_by_keywords("Annual budget report available to public")
    assert level == "public"
    assert conf >= 0.7
...
def test_handler_uses_keyword_fallback(classification_handler, lambda_context, monkeypatch):
    monkeypatch.setenv("OUTPUT_BUCKET", "test-bucket")
    monkeypatch.delenv("CLASSIFIER_ENDPOINT_ARN", raising=False)

    mock_comprehend = MagicMock()
    mock_comprehend.detect_dominant_language.return_value = {"Languages": [{"LanguageCode": "en", "Score": 0.99}]}
    ...
    with (
        patch.object(classification_handler, "boto3") as mock_boto3,
        patch.object(classification_handler, "OutputWriter") as mock_output_writer_cls,
    ):
