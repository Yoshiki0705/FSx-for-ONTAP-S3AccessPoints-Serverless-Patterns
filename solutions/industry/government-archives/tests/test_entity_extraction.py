...
def test_detect_pii_regex_email(entity_extraction_handler):
    entities = entity_extraction_handler.detect_pii_regex("Contact john.doe@example.com for details")
...
def test_detect_pii_regex_credit_card(entity_extraction_handler):
    entities = entity_extraction_handler.detect_pii_regex("Card: 1234-5678-9012-3456")
    assert any(e["Type"] == "CREDIT_CARD" for e in entities)


def test_detect_pii_regex_phone_us(entity_extraction_handler):
    entities = entity_extraction_handler.detect_pii_regex("Call 202-555-1234 for info")
...
def test_detect_pii_comprehend_success(entity_extraction_handler):
    ...
    entities = entity_extraction_handler.detect_pii_comprehend(mock_comp, "John Doe works here", "en")
...
def test_detect_pii_comprehend_empty_text(entity_extraction_handler):
    mock_comp = MagicMock()
    entities = entity_extraction_handler.detect_pii_comprehend(mock_comp, "", "en")
    assert entities == []


def test_handler_extracts_pii(entity_extraction_handler, lambda_context, monkeypatch):
    ...
    with (
        patch.object(entity_extraction_handler, "boto3") as mock_boto3,
        patch.object(entity_extraction_handler, "OutputWriter") as mock_output_writer_cls,
    ):
