...
def test_invoke_textract_cross_region_success(ocr_handler):
    ...
    with patch.object(ocr_handler, "CrossRegionClient", return_value=mock_client):
...
def test_invoke_textract_cross_region_fallback_to_same_region(ocr_handler):
    ...
    with (
        patch.object(ocr_handler, "CrossRegionClient", return_value=cross_region_mock),
...
def test_handler_reports_invoke_mode(ocr_handler, lambda_context, monkeypatch):
    ...
    mock_s3 = MagicMock()
    mock_s3.get_object.return_value = {"Body": MagicMock(read=lambda: b"%PDF-1.4" + b"\x00" * 500)}
    mock_textract = MagicMock()
    mock_textract.analyze_document.return_value = {"Blocks": [{"BlockType": "LINE", "Text": "test"}]}
