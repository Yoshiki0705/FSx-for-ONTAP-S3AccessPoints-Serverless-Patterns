...
> **注記**: UC16 の sample run は合成または非機微のサンプル文書を用いた検証であり、実際の行政文書や本番データを表すものではありません。本 sample run は処理パスの検証のみです。墨消し品質、Human Review の完全性、監査証跡の評価は、導入先固有の PoC で別途実施してください。
