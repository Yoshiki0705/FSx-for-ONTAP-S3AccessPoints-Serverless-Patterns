...
│                                                                              │
│  /vol/bim_projects/                                                          │
│  ├── models/building_A_v3.ifc         (IFC BIM モデル)                       │
│  ├── models/building_A_v3.rvt         (Revit ファイル)                       │
│  ├── drawings/floor_plan_1F.dwg       (AutoCAD 図面)                         │
│  └── drawings/safety_plan.pdf         (安全計画図面 PDF)                     │
│                                                                              │
└──────────────────────────────────┬───────────────────────────────────────────┘
                                   │
                                   ▼
...
│  • ListObjectsV2 (BIM/CAD ファイル検出)                                      │
│  • GetObject (IFC/RVT/DWG/PDF 取得)                                          │
...
│  │           │  │             │  │             │  │                 │   │
│  │ • VPC内    │  │ • IFC メタ  │  │ • Textract   │  │ • Bedrock        │   │
│  │ • S3 AP   │  │   データ抽出 │  │ • 図面テキス│  │ • 安全コンプラ  │   │
│  │ • IFC/RVT │  │ • バージョン│  │   ト抽出    │  │   イアンスチェッ│   │
│  │   /DWG/PDF│  │   差分検出  │  │             │  │   ク            │   │
│  └───────────┘  └──────────────┘  └──────────────┘  └──────────────────┘   │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
...
│  │   └── building_A_v3.json          ← BIM メタデータ + 差分                │
│  ├── ocr-text/YYYY/MM/DD/                                                    │
│  │   └── safety_plan.json            ← OCR 抽出テキスト・テーブル           │
│  └── compliance/YYYY/MM/DD/                                                  │
│      └── building_A_v3_safety.json   ← 安全コンプライアンスレポート         │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
