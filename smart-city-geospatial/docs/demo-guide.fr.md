...
```bash
# Téléchargement d'une photo aérienne d'exemple (quartier de Sendai)
...
- `s3://<out>/landuse/gis/2026/05/sendai.tif.json` (répartition de l'occupation des sols)
...
- Affichage du Markdown du rapport généré par Bedrock
...
- Traitement de nuages de points en production (déploiement de la couche LAS)
- Prochaines étapes : intégration MapServer, portail citoyen

## Questions fréquentes et réponses

**Q. La conversion CRS est-elle réellement effectuée ?**  
R. Uniquement lors du déploiement de la couche rasterio / pyproj. Repli avec vérification `PYPROJ_AVAILABLE`.

**Q. Critères de sélection du modèle Bedrock ?**  
R. Nova Lite offre un bon équilibre coût/précision. Claude Sonnet recommandé pour les textes longs.
...
**Charges de travail concernées** : métadonnées de normalisation CRS / classification de l'occupation des sols / évaluation des infrastructures / cartes de risque / rapports générés par Bedrock
...
Même approche que pour les démos Phase 7 UC15/16/17 et UC6/11/14 : cibler **les écrans UI/UX que les
utilisateurs finaux voient réellement dans leur travail quotidien**. Les vues techniques (graphe Step Functions, événements
de pile CloudFormation, etc.) sont consolidées dans `docs/verification-results-*.md`.
...
1. **Préparation** :
   - Vérifier les prérequis avec `bash scripts/verify_phase7_prerequisites.sh` (VPC commun/S3 AP présents)
   - Créer le package Lambda avec `UC=smart-city-geospatial bash scripts/package_generic_uc.sh`
   - Déployer avec `bash scripts/deploy_generic_ucs.sh UC17`

2. **Placement des données d'exemple** :
   - Télécharger un GeoTIFF d'exemple via l'Alias S3 AP avec le préfixe `gis/`
   - Démarrer Step Functions `fsxn-smart-city-geospatial-demo-workflow` (entrée `{}`)

3. **Capture** (fermer CloudShell/terminal, masquer le nom d'utilisateur en haut à droite du navigateur) :
   - Vue d'ensemble du bucket de sortie S3 `fsxn-smart-city-geospatial-demo-output-<account>`
   - Aperçu du rapport Markdown Bedrock dans le navigateur
   - Liste des éléments de la table DynamoDB landuse_history
   - Vérification de la structure JSON de la carte des risques

4. **Traitement de masquage** :
   - Masquage automatique avec `python3 scripts/mask_uc_demos.py smart-city-geospatial-demo`
