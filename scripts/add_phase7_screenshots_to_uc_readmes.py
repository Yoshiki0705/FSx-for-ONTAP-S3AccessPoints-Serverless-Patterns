...
UC_SECTIONS: dict[str, dict[str, list[tuple[str, str, str]]]] = {
    ...
    "defense-satellite": {
        ...
        "en": [
            ...
            ("#### 3. Change-Detection Alert (SNS Email)", "UC15: SNS alert email", "phase7-uc15-sns-alert-email.png"),
            ("#### 4. Detection Results (JSON)", "UC15: Detection results JSON", "phase7-uc15-detections-json.png"),
        ],
        "ko": [
            ("### 검증된 UI/UX 스크린샷", "", ""),
            (
            ...
            ("#### 2. 분석 결과 (S3 출력 버킷)", "UC15: S3 출력 버킷", "phase7-uc15-s3-output-bucket.png"),
            ("#### 3. 변화 감지 경보 (SNS 이메일)", "UC15: SNS 경보 이메일", "phase7-uc15-sns-alert-email.png"),
            ("#### 4. 탐지 결과 (JSON)", "UC15: 탐지 결과 JSON", "phase7-uc15-detections-json.png"),
        ],
        "zh-CN": [
            ("### 已验证的 UI/UX 截图", "", ""),
            (
            ...
            ("#### 2. 分析输出（S3 输出桶）", "UC15: S3 输出桶", "phase7-uc15-s3-output-bucket.png"),
            ("#### 3. 变化检测告警（SNS 电子邮件）", "UC15: SNS 告警邮件", "phase7-uc15-sns-alert-email.png"),
            ("#### 4. 检测结果（JSON）", "UC15: 检测结果 JSON", "phase7-uc15-detections-json.png"),
        ],
        "zh-TW": [
            ("### 已驗證的 UI/UX 螢幕截圖", "", ""),
            (
            ...
            ("#### 2. 分析輸出（S3 輸出儲存貯體）", "UC15: S3 輸出儲存貯體", "phase7-uc15-s3-output-bucket.png"),
            ("#### 3. 變化偵測警報（SNS 電子郵件）", "UC15: SNS 警報郵件", "phase7-uc15-sns-alert-email.png"),
            ("#### 4. 偵測結果（JSON）", "UC15: 偵測結果 JSON", "phase7-uc15-detections-json.png"),
        ],
        "fr": [
            ...
            ("#### 4. Résultats de détection (JSON)", "UC15: Résultats JSON", "phase7-uc15-detections-json.png"),
        ],
        "de": [
            ...
            ("#### 3. Veränderungs-Alarm (SNS-E-Mail)", "UC15: SNS-Alarm-E-Mail", "phase7-uc15-sns-alert-email.png"),
            (
        ...
        "es": [
            ...
            ("#### 2. Salida de análisis (Bucket S3)", "UC15: Bucket S3 de salida", "phase7-uc15-s3-output-bucket.png"),
            (
    ...
    "government-archives": {
        ...
        "ko": [
            ("### 검증된 UI/UX 스크린샷", "", ""),
            ("#### 1. 공문서 배치 (S3 AP 경유)", "UC16: 공문서 배치", "phase7-uc16-s3-archives-uploaded.png"),
            ("#### 2. 편집된 문서 미리보기", "UC16: 편집된 문서 미리보기", "phase7-uc16-redacted-text-preview.png"),
            (
            ...
            ("#### 5. NARA GRS 보존 일정 (DynamoDB)", "UC16: 보존 테이블", "phase7-uc16-dynamodb-retention.png"),
        ],
        "zh-CN": [
            ("### 已验证的 UI/UX 截图", "", ""),
            ("#### 1. 公文档案放置（通过 S3 AP）", "UC16: 公文档案放置", "phase7-uc16-s3-archives-uploaded.png"),
            ("#### 2. 已编辑文档预览", "UC16: 已编辑文档预览", "phase7-uc16-redacted-text-preview.png"),
            ("#### 3. 编辑元数据（Sidecar JSON）", "UC16: 编辑元数据", "phase7-uc16-redaction-metadata-json.png"),
            ("#### 4. FOIA 期限提醒（SNS 电子邮件）", "UC16: FOIA 提醒邮件", "phase7-uc16-foia-reminder-email.png"),
            ("#### 5. NARA GRS 保存计划（DynamoDB）", "UC16: 保存表", "phase7-uc16-dynamodb-retention.png"),
        ],
        "zh-TW": [
            ("### 已驗證的 UI/UX 螢幕截圖", "", ""),
            ("#### 1. 公文檔案放置（透過 S3 AP）", "UC16: 公文檔案放置", "phase7-uc16-s3-archives-uploaded.png"),
            ("#### 2. 已編輯文件預覽", "UC16: 已編輯文件預覽", "phase7-uc16-redacted-text-preview.png"),
            ("#### 3. 編輯中繼資料（Sidecar JSON）", "UC16: 編輯中繼資料", "phase7-uc16-redaction-metadata-json.png"),
            ("#### 4. FOIA 期限提醒（SNS 電子郵件）", "UC16: FOIA 提醒郵件", "phase7-uc16-foia-reminder-email.png"),
            ("#### 5. NARA GRS 保存排程（DynamoDB）", "UC16: 保存表", "phase7-uc16-dynamodb-retention.png"),
        ],
        "fr": [
            ("### Captures d'écran UI/UX vérifiées", "", ""),
            (
            ...
            ("#### 2. Aperçu du document rédigé", "UC16: Document rédigé", "phase7-uc16-redacted-text-preview.png"),
            (
        ...
        "de": [
            ("### Verifizierte UI/UX-Screenshots", "", ""),
            ("#### 1. Aktenablage (über S3 AP)", "UC16: Aktenablage", "phase7-uc16-s3-archives-uploaded.png"),
            (
    ...
    "smart-city-geospatial": {
        "en": [
            ...
            ("#### 3. Disaster Risk Map (JSON)", "UC17: Risk map", "phase7-uc17-risk-map-json.png"),
            ("#### 4. Land-Use Distribution", "UC17: Land-use distribution", "phase7-uc17-landuse-distribution.png"),
            (
        ...
        "ko": [
            ("### 검증된 UI/UX 스크린샷", "", ""),
            ("#### 1. GIS 데이터 배치 (S3 AP 경유)", "UC17: GIS 데이터 배치", "phase7-uc17-s3-gis-uploaded.png"),
            ("#### 2. Bedrock 이 생성한 도시 계획 보고서", "UC17: Bedrock 보고서", "phase7-uc17-bedrock-report.png"),
            ("#### 3. 재난 위험 지도 (JSON)", "UC17: 위험 지도", "phase7-uc17-risk-map-json.png"),
            ("#### 4. 토지 이용 분포", "UC17: 토지 이용 분포", "phase7-uc17-landuse-distribution.png"),
            ("#### 5. 시계열 변화 (DynamoDB)", "UC17: 토지 이용 이력", "phase7-uc17-dynamodb-landuse-history.png"),
        ],
        "zh-CN": [
            ("### 已验证的 UI/UX 截图", "", ""),
            ("#### 1. GIS 数据放置（通过 S3 AP）", "UC17: GIS 数据放置", "phase7-uc17-s3-gis-uploaded.png"),
            ("#### 2. Bedrock 生成的城市规划报告", "UC17: Bedrock 报告", "phase7-uc17-bedrock-report.png"),
            ("#### 3. 灾害风险地图（JSON）", "UC17: 风险地图", "phase7-uc17-risk-map-json.png"),
            ("#### 4. 土地利用分布", "UC17: 土地利用分布", "phase7-uc17-landuse-distribution.png"),
            ("#### 5. 时间序列变化（DynamoDB）", "UC17: 土地利用历史", "phase7-uc17-dynamodb-landuse-history.png"),
        ],
        "zh-TW": [
            ("### 已驗證的 UI/UX 螢幕截圖", "", ""),
            ("#### 1. GIS 資料放置（透過 S3 AP）", "UC17: GIS 資料放置", "phase7-uc17-s3-gis-uploaded.png"),
            ("#### 2. Bedrock 產生的都市計劃報告", "UC17: Bedrock 報告", "phase7-uc17-bedrock-report.png"),
            ("#### 3. 災害風險地圖（JSON）", "UC17: 風險地圖", "phase7-uc17-risk-map-json.png"),
            ("#### 4. 土地利用分佈", "UC17: 土地利用分佈", "phase7-uc17-landuse-distribution.png"),
            ("#### 5. 時間序列變化（DynamoDB）", "UC17: 土地利用歷史", "phase7-uc17-dynamodb-landuse-history.png"),
        ],
        "fr": [
            ("### Captures d'écran UI/UX vérifiées", "", ""),
            ("#### 1. Placement de données GIS (via S3 AP)", "UC17: Placement GIS", "phase7-uc17-s3-gis-uploaded.png"),
            (
            ...
            ("#### 4. Distribution d'usage du sol", "UC17: Usage du sol", "phase7-uc17-landuse-distribution.png"),
            (
        ...
        "de": [
            ...
            ("#### 3. Katastrophen-Risikokarte (JSON)", "UC17: Risikokarte", "phase7-uc17-risk-map-json.png"),
            ("#### 4. Landnutzungsverteilung", "UC17: Landnutzungsverteilung", "phase7-uc17-landuse-distribution.png"),
            ("#### 5. Zeitreihe (DynamoDB)", "UC17: Nutzungshistorie", "phase7-uc17-dynamodb-landuse-history.png"),
        ],
        "es": [
            ("### Capturas de pantalla UI/UX verificadas", "", ""),
            (
            ...
            ("#### 3. Mapa de riesgos de desastres (JSON)", "UC17: Mapa de riesgos", "phase7-uc17-risk-map-json.png"),
            (
...
def apply_to_readme(uc: str, locale: str, entries: list[tuple[str, str, str]]) -> None:
    ...
    if (
        "### Verified UI/UX Screenshots" in content
        or "### 검증된 UI/UX 스크린샷" in content
        or "### 已验证的 UI/UX 截图" in content
        or "### 已驗證的 UI/UX 螢幕截圖" in content
        or "### Captures d'écran UI/UX vérifiées" in content
        or "### Verifizierte UI/UX-Screenshots" in content
        or "### Capturas de pantalla UI/UX verificadas" in content
    ):
