...
🌐 **Language / 言語**: 日本語 | [English](trigger-mode-decision-guide.en.md)
...
| **EVENT_DRIVEN** | ニアリアルタイム取り込みが必要で、再接続時のイベントロスが許容可能。**かつ書き込みが NFS / SMB 経由で届く** | **書き込みが S3 Access Point 経由で届く**（AP 経由の操作は FPolicy 通知を発火しない。実測 2026-08-26 / ONTAP 9.18.1P3D1）。コンプライアンスが耐久性のあるイベントキャプチャを要求 |
...
| **対応プロトコル** | 全て（S3 AP 経由） | NFSv3/NFSv4.0/NFSv4.1/SMB のみ。**S3 AP 経由の操作は検知しない** | 全て。ただし S3 AP 経由の書き込みを拾うのはポーリング側だけで、秒単位にはならない |
