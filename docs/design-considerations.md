...
4. **ONTAP Snapshot による整合性ポイント**: バッチ処理の入力データを Snapshot で固定し、処理中の変更を排除
...
> ファイル操作の主体を後から分離できません。**SVM を AD に参加させても解決しません** — 実在の
> ドメインアカウントを固定した AP でも `SubjectUserName` は `Not Present` のままでした（実測
> 2026-08-19）。
