...
- [FSx for ONTAP + Bedrock RAG ブログ](https://aws.amazon.com/blogs/machine-learning/build-rag-based-generative-ai-applications-in-aws-using-amazon-fsx-for-netapp-ontap-with-amazon-bedrock/)
...
- SFTP 接続: FSx for ONTAP 上の全ディレクトリ/ファイル一覧表示
- ファイルアップロード（put）: S3 AP 経由で FSx for ONTAP に書き込み
...
```
外部パートナー → Transfer Family SFTP → FSx for ONTAP
