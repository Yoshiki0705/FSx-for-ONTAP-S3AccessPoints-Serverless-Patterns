...
| U10 | **S3 AP のリクエスト単位のメトリクスが AWS の機能として存在しないのか、ドキュメントに書かれていないだけなのか**（§10.2） | **AWS サポートに確認済み（2026-09-06 起票、回答待ち）[E-001]。** 回答が「機能として無い」なら機能改善要望として出す。公開物にはケース番号やベンダー内部 ID を書かず、トピックとしてのみ言及する |
