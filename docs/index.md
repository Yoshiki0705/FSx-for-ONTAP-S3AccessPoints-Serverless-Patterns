...
> このリポジトリは **FSx for ONTAP S3 Access Point の実装パターン**を持ちます。
> **仕様・上限値・設計判断の根拠**は [FSx for ONTAP Adoption Playbook](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook) 側にあります。
...
| 本番に出す前に読む | 各パターン README 末尾の「本番に出す前に読むもの」 |
| 運用でコストと容量を下げる | [operations/](../operations/) の OPS1-6 |
| 利用者にブラウザから使わせる | [ファイルポータル](../solutions/amplify-portal/) |
| 実際に踏んだ罠を引く | [作業別のノート](#作業別のノートdocsagent) |

## 関連プロジェクト

**このリポジトリだけでは完結しません。** 分担は次のとおりです。

| プロジェクト | 持っているもの |
|---|---|
| [FSx-for-ONTAP-Adoption-Playbook](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook) | **仕様・上限値・設計判断の根拠。** 業種別に「最初に読むモジュール」を示す読む順序、決定ツリー、実測した制約 |
| **このリポジトリ** | **動く実装。** 業種別 UC 28 + FlexCache/FlexClone 10 + 運用最適化 6 + ファイルポータル + GenAI / エッジ / イベント駆動 / SAP / HA |
| [S3-Burst-on-ONTAP-Files](https://github.com/Yoshiki0705/S3-Burst-on-ONTAP-Files) | S3 で収集 → FlexCache の NFS/SMB で利用。性能の実測値の出典 |
| [FSx-for-ONTAP-Observability-integrations](https://github.com/Yoshiki0705/FSx-for-ONTAP-Observability-integrations) | 監査ログの外部 SIEM 連携 |
| [FSx-for-ONTAP-Lakehouse-Integrations](https://github.com/Yoshiki0705/FSx-for-ONTAP-Lakehouse-Integrations) | Athena / Glue / Spark からの S3 AP 経由アクセス |
| [FSx-for-ONTAP-Cyber-Resilience-Patterns](https://github.com/Yoshiki0705/FSx-for-ONTAP-Cyber-Resilience-Patterns) | ARP + File Security + FPolicy の多層防御 |
| [FSx-for-ONTAP-Agentic-Access-Aware-RAG](https://github.com/Yoshiki0705/FSx-for-ONTAP-Agentic-Access-Aware-RAG) | アクセス制御対応の Agentic RAG (AWS CDK) |
...
どのパターンを deploy するか、いくらかかるか、PoC をどう通すか。
...
| [App foundation choices (EN)](en/scaffolding-and-backend-toolkit-choices.md) / [アプリの構築ツールの選択肢 (JA)](ja/scaffolding-and-backend-toolkit-choices.md) | Nx Plugin for AWS / AWS Blocks / Amplify Gen 2: measured resource counts and generated defaults for an equivalent starter, the fixed cost those defaults imply, how to choose, and why the portal is Amplify Gen 2 |
| [Deploy verification and teardown (EN)](en/scaffolding-deploy-verification.md) / [デプロイ検証と後片付けの手順 (JA)](ja/scaffolding-deploy-verification.md) | Deploying those same three outputs to AWS: the pitfall register mapped to upstream sources, the teardown order for deletion-protected and `Retain` resources, and what was measured live |
| [The same four features on three foundations (EN)](en/portal-parity-four-features.md) / [同じ 4 機能を 3 つの構築ツールで実装した記録 (JA)](ja/portal-parity-four-features.md) | Sign in, list, read and upload implemented on all three and run against one FSx for ONTAP S3 Access Point: how the permission differs, which layer refuses a malformed write, twelve captures side by side, and the pitfalls |
| [Beyond the four features (EN)](en/portal-parity-next-steps.md) / [4 機能の先へ (JA)](ja/portal-parity-next-steps.md) | Six stages past the four features, what each newly requires, where a VPC attachment and a standing charge first appear, and which stages can be skipped |
...
利用者にブラウザから使わせる構成。ドキュメントが最も多い領域。
...
何をどこまで実機で確認したか。数値の出典。

| Document | Purpose |
|----------|---------|
| [監査ログボリューム — コンソールでの操作ポイント (JA)](ja/snaplock-audit-log-console-guardrails.md) | 実画面 3 枚。作成画面の警告が「ボリュームまで」で SVM とファイルシステムに触れないこと、保持期間の 3 欄が WORM ファイル用で監査ログの指定欄が無いこと、詰まった後に読むべきは `ライフサイクル移行メッセージ` であること、満了日時の読み方、2 つのフィールドの役割 |
