...
- 複数テンプレートで試す（Claude Haiku は別 ARN `anthropic.claude-haiku-4-5-20251001-v1:0`）
...
**対処**: UC 単位で実行する:
```bash
python3 -m pytest solutions/industry/defense-satellite/tests/ -q
python3 -m pytest solutions/industry/government-archives/tests/ -q
python3 -m pytest solutions/industry/smart-city-geospatial/tests/ -q
```
...
渡すが、FSx for ONTAP S3 AP の実装では ARN 形式での list 呼び出しが不安定な場合がある（2026-05 時点）。
...
これは FSx for ONTAP S3 AP に書き出す「no data movement」のコスト的魅力を補強する:
...
**症状**: FSx for ONTAP S3 AP permission 判定は alias 形式（`arn:aws:s3:::<alias>/*`）と
...
**予防策**: 新 UC を追加するときは `solutions/industry/autonomous-driving/template-deploy.yaml`
