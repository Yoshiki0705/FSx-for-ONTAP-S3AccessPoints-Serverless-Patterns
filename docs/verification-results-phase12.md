...
**問題**: ECS タスクロールの SQS ポリシーで Resource ARN パターン `arn:aws:sqs:ap-northeast-1:123456789012:fsxn-fpolicy-*` が実際のキュー名 `FPolicy_Q` にマッチしなかった。
...
aws cloudformation deploy \
  ...
  --parameter-overrides EnableSLODashboard=true ProjectPrefix=fsxn-s3ap \
    SnsTopicArn=arn:aws:sns:ap-northeast-1:123456789012:fsxn-s3ap-aggregated-alerts \
  --region ap-northeast-1
...
aws cloudformation package \
  ...
  --s3-bucket fsxn-eda-deploy-123456789012 \
...
aws cloudformation deploy \
  ...
  --parameter-overrides EnableCapacityForecast=true ProjectPrefix=fsxn-s3ap \
    FileSystemId=fs-0123456789abcdef0 TotalCapacityGb=1024 \
    SnsTopicArn=arn:aws:sns:ap-northeast-1:123456789012:fsxn-s3ap-aggregated-alerts \
  --capabilities CAPABILITY_NAMED_IAM \
  --region ap-northeast-1
...
aws cloudformation deploy \
  ...
  --parameter-overrides EnableSecretsRotation=true ProjectPrefix=fsxn-s3ap \
    SecretArn=arn:aws:secretsmanager:ap-northeast-1:123456789012:secret:fsx-ontap-fsxadmin-credentials-P9Ibbi \
...
aws s3 cp canary-code/s3ap_health_check.zip \
  s3://fsxn-eda-deploy-123456789012/canary-code/s3ap_health_check.zip
...
aws cloudformation deploy \
  ...
  --parameter-overrides EnableSyntheticMonitoring=true ProjectPrefix=fsxn-s3ap \
    ...
    SnsTopicArn=arn:aws:sns:ap-northeast-1:123456789012:fsxn-s3ap-aggregated-alerts \
    ArtifactBucket=fsxn-eda-deploy-123456789012 \
  --capabilities CAPABILITY_NAMED_IAM \
  --region ap-northeast-1
```
