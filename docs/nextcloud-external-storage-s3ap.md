...
## CLI 自動化セットアップ（Docker / occ コマンド）

GUI を使わずに CLI で External Storage を設定する場合（CI/CD、Docker 環境、スクリプト化）:

```bash
# 1. External Storage アプリ有効化
...
# → "Storage created with id N" と表示される。N を控える
...
> **CLI の落とし穴**:
> - `occ files_external:config` は **1パラメータにつき1回の実行**が必要。複数の `-c` フラグや引数を1コマンドにまとめると "Too many arguments" エラーになる
> - Docker 実行時は `-u 33` (www-data ユーザー) が必須。root で実行するとパーミッションエラー
> - 認証情報（key/secret）が空の場合、AWS SDK は EC2 Instance Metadata (169.254.169.254) にフォールバックし、Docker 内ではタイムアウトする

本リポジトリの `solutions/nextcloud-test/` に Docker Compose + Makefile による自動化セットアップが含まれています:
