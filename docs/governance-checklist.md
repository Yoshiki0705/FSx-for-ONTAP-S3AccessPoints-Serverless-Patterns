...
- **アクセス制御**: AWS IAM とファイルシステム権限の二層認可が設計されている。**AWS 側で絞り込みを担うのは明示的な拒否**であり、AP ポリシーの `Allow` を狭くすることは絞り込みにならない（同一アカウントでは identity-based ポリシーと結合して評価される）
- **監査可能性**: 処理履歴・アクセスログ・データリネージが保持される。**ONTAP のファイルアクセス監査に記録されるのは S3 AP に固定した ID であり、呼び出し元の IAM プリンシパルではない** — 主体別の追跡が要件なら、AP の用途別分割か AWS CloudTrail との突き合わせで担保する（**SVM の AD 参加では解決しません**。実在のドメインアカウントでも `SubjectUserName` は `Not Present`）
...
> **注意**: 上記の職務分掌は一般的な原則です。実案件では各組織の規程、監査要件、人員体制に合わせて調整してください。小規模組織では一部ロールを兼務する場合がありますが、その場合は兼務の理由と代替統制を文書化することを推奨します。
...
ガバナンスチェックは導入時だけでなく、定期的に再確認する必要があります:

| レビュータイミング | 確認内容 |
|-----------------|---------|
| 四半期ごと | AI 出力品質レビュー、Human Review 率の確認、誤分類・誤要約の傾向分析 |
| 本番変更時 | データ分類の変更、新規 UC 追加、AI モデル変更時のガバナンス再確認 |
| 年次 | コンプライアンスマッピングの更新、監査証跡の保持期間確認、規制変更への対応 |
| インシデント発生時 | 根本原因分析、Human-in-the-loop プロセスの見直し、再発防止策 |

> **注意**: 上記のレビュー頻度は一般的な目安です。実案件では組織の規程、監査要件、データ分類レベル、システム重要度に合わせて調整してください。
