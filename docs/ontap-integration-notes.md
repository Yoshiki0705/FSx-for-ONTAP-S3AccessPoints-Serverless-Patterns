...
| パターンタイプ | 推奨境界 |
|---|---|
| 低リスク Demo / PoC | 既存 volume 内の専用プレフィックス |
| PII / HR ワークフロー (UC27) | 専用 volume（アクセス制限 + 監査） |
| 安全重要点検 (UC22/UC25) | 専用出力 volume + review プレフィックス |
| マルチチーム共有データ | SVM 分離 or チーム別 S3 AP |
| 規制対応ワークロード | 専用 volume + SnapLock 保持 + 監査ポリシー |
...
> **SMB 移行に関する補足**: Windows ファイルサーバーからの ACL 付きデータ移行で、コピー実行ユーザーに
> ACL 上の権限がないファイルを扱う方法は [Backup Operators 特権による ACL 付きデータコピー](./smb-acl-migration-backup-operators.md) を参照。
...
- IAM Access Analyzer で S3 AP policy を本番前に検証
