...
| Presigned PUT answers `301 PermanentRedirect` | The signature was v4 but the URL named the global endpoint: under the default `addressing_style` botocore presigns `<alias>.s3.amazonaws.com` even with a region set, and the redirect cannot be followed because the signature covers `host`. Name both: `Config(signature_version="s3v4", s3={"addressing_style": "virtual"})` → `<alias>.s3.<region>.amazonaws.com`. Measured 2026-08-15 |
| Which presign shape to use | Both work against an S3 AP alias, measured 2026-08-15: path-style with an explicit regional `endpoint_url` (what six of the portal's seven presigning functions do) and virtual addressing via `addressing_style`. Only leaving both to the default fails. `make drift` rejects that combination in any handler that presigns |
