...
| S3 AP creation → `FAILED`, "existing ONTAP object storage server on SVM" | The SVM runs a **native ONTAP S3 service** (its buckets appear as `fg_oss_*` FlexGroups). FSx S3 APs cannot coexist with it on the same SVM. Check `GET /protocols/s3/services?svm.name=<svm>` before creating; use a different SVM |
