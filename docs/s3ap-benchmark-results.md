...
> **Sizing signal**: 主要な設計指標は平均レイテンシではなく tail latency (P99) です。このテスト環境では、concurrency=10 を超えると P99 が急激に増加しました。concurrency=1 では P95/P99 が最大値に近い値となりますが、これはサンプル数が少ないためです。
