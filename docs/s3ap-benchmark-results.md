...
🌐 **Language / 言語**: 日本語 | [English](s3ap-benchmark-results.en.md)
...
> **環境制約**: 全結果は Single-AZ, First-generation FSx for ONTAP からの計測です。Multi-AZ または Second-generation ファイルシステムでは異なる結果になる可能性があります。個別に検証してください。
...
- VPC 内 Lambda（VPC-origin S3 AP 経由）ではさらに高スループットが期待されるが、未測定
...
> **注記**: 256/512 MBps 検証は 2026-05-25（1 MB ファイル）および 2026-06-06（202 bytes ファイル）に実施完了。FSx throughput capacity 増加は大ファイル (1 MB+) の tail latency 改善に有効だが、小ファイルでは効果なし。VPC 内 Lambda テストが次の優先検証事項。
...
**観測日**: 2026-05-23
**環境**: fs-0123456789abcdef0 (SINGLE_AZ_1, ap-northeast-1)
...
### 検証結果（2026-06-06 追加検証）

**小ファイル (202 bytes) テスト結果**: Throughput capacity の増加は小ファイルの P50 レイテンシに影響しない（256 MBps と 512 MBps で P50 ≈ 57-60 ms @ concurrency ≤25）。ボトルネックは接続オーバーヘッド（TLS + S3 AP ルーティング）であり、FSx 帯域ではない。

**Updated Conclusion**: 
- 仮説は **ファイルサイズに依存して部分的に支持** される
- 大ファイル (1 MB+): Throughput capacity 増加で P99 が改善（128→256 MBps で 51% 改善 @ concurrency=20）
- 小ファイル (< 1 KB): Throughput capacity は P50/P99 に影響しない（接続オーバーヘッド支配）
- インターネット経由テストの限界: 512 MBps の効果はクライアント帯域で隠される
...
> **注意**: VPC-external Lambda（VpcConfig なし）から Internet-origin S3 AP へのアクセスです。AWS マネージド Lambda egress 経路であり、true VPC-internal パス（VPC-origin S3 AP）ではありません。
...
| オブジェクトサイズ | 202 bytes (JSON manifest) |
| 反復回数 | 50 iterations per concurrency level |
| ウォームアップ | 3 requests |
| クライアント | macOS (boto3, Python 3.12) — インターネット経由 |
| 計測日 | 2026-06-06 |

> **目的**: 小ファイル（接続オーバーヘッドが支配的）で throughput capacity 変更の効果を確認。大ファイル (1 MB) テスト (2026-05-25) との比較。

### GetObject — 202 bytes ファイル（256 MBps）
...
### GetObject — 202 bytes ファイル（512 MBps）
...
**P50 比較（202 bytes GetObject）**:

| Concurrency | 256 MBps P50 | 512 MBps P50 | 差異 |
|:-----------:|:---:|:---:|:---:|
| 1 | 56.9 ms | 59.8 ms | ≈同等 |
| 10 | 56.5 ms | 57.2 ms | ≈同等 |
| 25 | 60.3 ms | 59.9 ms | ≈同等 |
