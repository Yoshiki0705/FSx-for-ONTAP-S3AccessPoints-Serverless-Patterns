...
🌐 **Language / 言語**: [日本語](s3ap-compatibility-notes.md) | English
...
| Event-driven file processing | △ (EventBridge Scheduler polling. **FPolicy does not see operations through the S3 access point** — measured 2026-08-26, ONTAP 9.18.1P3D1) | ✅ (FPolicy + NFS/SMB) |
...
| `MalformedPolicy: Policy has invalid action` on put-access-point-policy | An action the AP policy does not accept ([the 20 measured](s3ap-authorization-model.en.md#actions-that-cannot-be-used-in-an-access-point-policy)). **The message does not name which one** | Apply them one at a time to isolate it. Move bucket-configuration actions (`GetBucketLocation` etc.) and access-point-management actions (`PutAccessPointPolicy`) to the identity-based policy | All |
