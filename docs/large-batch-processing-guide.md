...
### FSx ONTAP スループット共有への配慮

MapConcurrency を高く設定すると、FSx ONTAP の Throughput Capacity を圧迫する可能性があります:

- FSx ONTAP の Throughput Capacity は NFS/SMB/S3 AP で**共有**
...
## FSx ONTAP Throughput Capacity 別の推奨 MapConcurrency 上限

S3 AP 経由のスループットは FSx ONTAP の Throughput Capacity に制約されます。以下はプロビジョニング済み Throughput Capacity ごとの MapConcurrency 上限目安です。
