...
`COPY` モードのまま運用します。現時点で `REFERENCE` モードを必要とするパターンが無いため、リポジトリ全体のパッケージング規約を変更する理由がありません。必要が生じた場合は、全テンプレートを書き換えるのではなく、上記の**サポートされる回避策**（ネイティブ `AWS::Lambda::Function`）を該当する関数にだけ適用します。
