...
**当ポータルの列は実装を読んで判定しています**（該当コミット時点）。比較対象の列は各サービスの公式ドキュメントに基づく記載で、粒度の判定までは行っていません。したがって比較対象の ✅ と当ポータルの ✅ は厳密には同じ検証強度ではありません。

### Gap Matrix — 基本ファイル管理機能

> **データ鮮度**: 2025-07 〜 2026-07 の公式ドキュメント・リリースノートに基づく。各サービスの機能は急速に変化するため、最新状況は各社公式サイトを参照してください。
> **参考文献マッピング**: Storage Browser [1][2][3], FSx for ONTAP S3 AP 互換性 [4], Transfer Family [5][6][7], Presigned URL [13], Box Retention [14], ファイルポータル要件 [15][16]
...
> **データ鮮度**: Box Agent GA (2026/4), SharePoint Copilot 拡張プレビュー (2026/7), Google Gemini Drive 統合 (2026), Dropbox Dash (2025)。各社の AI 機能は月単位で更新されるため、比較は 2026-07 時点のスナップショットです。
> **参考文献マッピング**: Bedrock RAG [8], Amazon Quick [11][12], Kendra 廃止 [9], Q Business 停止 [10]
...
> **データ鮮度**: 各製品の公式セキュリティページおよび認証ステータスに基づく（2026-07 確認）。認証ステータス（FedRAMP, ISMAP 等）は年度更新のため、最新の認証状況は各機関の公開リストで確認してください。
> **参考文献マッピング**: Box Retention/Governance [14]
...
> **データ鮮度**: Transfer Family の FSx for ONTAP S3 AP 対応は 2026/1 GA [5][6][7]。Nextcloud External Storage の S3 互換性は Nextcloud 29 で検証済み。ローカル検証スタックは Nextcloud 34（`files_external` 1.26.0）へ更新済みで、`amazons3` バックエンドは引き続き提供されています（データ操作の再検証は未実施）。ownCloud OCIS の multi-storage は OCIS 5.x ドキュメント参照。
> **参考文献マッピング**: Transfer Family [5][6][7], S3 AP 互換性 [4]
...
### プロトコルアクセシビリティの詳細 — マルチプロトコルが重要な理由
...
> **データ鮮度**: 各サービスの価格は 2026-07 時点の公開料金ページに基づく。為替レートや契約形態（年間一括 vs 月額）により実コストは変動します。Wasabi の料金は [wasabi.com/pricing](https://wasabi.com/pricing) を参照。
> **参考文献マッピング**: エンタープライズファイル共有ガイド [15][16]
...
**このエラーが返る条件は実測しました。** UNIX identity の AP に対し、実在する IAM ロールを `Principal` とする AP ポリシーは正常に適用でき、`Deny` + `Condition` も適用できました。`Invalid principal in policy` が返ったのは、**存在しないロール ARN を指定した場合だけ**です（実測 2026-08-18/19, `ap-northeast-1`, ONTAP 9.18.1P3D1）。

**2026-06-12 に何が存在しなかったかは未確認です。** 参照したロールが当時まだ作成されていなかった、ARN が誤っていた、といった可能性が残ります。**当時の環境は再現できないため、そこは特定していません。**
