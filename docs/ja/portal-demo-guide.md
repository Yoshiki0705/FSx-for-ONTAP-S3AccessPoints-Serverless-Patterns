...
> **エンドユーザーの方へ**: ポータルがデプロイ済みで使い方だけ知りたい場合は、[ユーザーガイド](portal-user-guide.md) をご覧ください。
...
`storage-admin` グループのユーザーには管理セクションが出ます。上はボリューム一覧で、対象 SVM の
切り替えとボリューム作成が同じ画面から行えます。**既知の制約**: この幅では表のヘッダーが 1 文字
ずつ縦に折り返し、行も横に切れます。読めますが快適ではないので、一覧を精査する操作は横向きか
デスクトップを勧めます。緊急時に状態を見て 1 つ操作する、という使い方には足ります。
