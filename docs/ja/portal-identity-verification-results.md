...
## 実測 2: presigned URL の実行 identity — 署名した Access Point
...
### 4-1. SVM の既存 S3 サーバーによる Access Point 作成の阻害
...
### 4-4. NTFS ACL による identity 単位の判別
...
### 誤ったパスワードとロックアウト中に共通する ONTAP の「User is not authorized」応答
...
### 空のときの `discovered_servers` フィールドの省略
