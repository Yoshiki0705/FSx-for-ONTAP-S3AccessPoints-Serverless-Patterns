...
**実行可能なコマンドの一覧は見つけられませんでした**（2026-08-29 時点。存在するとしても FSx for ONTAP の CLI ドキュメントからは辿れません）。ただし列挙する手段があります。

```
security login role show -role fsxadmin -access !none
```

`fsxadmin` に許可されたコマンドが返るため、使いたいコマンドが範囲内かを事前に判定できます。ドキュメントを探すより確実です。

### 権限レベルの区別

コマンドによって要求される権限が異なります（2026-08-29 に `security login role show -role fsxadmin -access !none` の出力から読み取り）。

| コマンド | 必要な権限 |
|---|---|
| SMB セッションの表示・切断 | Admin |
| `vserver locks show` | Admin |
| `vserver locks break` | **Advanced** |

> **未確認**: FSx for ONTAP の `fsxadmin` は権限が制限されているため、**`set -privilege advanced` へ遷移して `vserver locks break` を実行できるかは確認していません**。到達できない場合、ロック解除の運用手順を別の方法で設計する必要があります。AWS に照会中です。

### クォータ

| 項目 | 内容 | 位置付け |
|---|---|---|
| 1 ボリューム当たりのクォータルール数 | 4,995 | **AWS が自社環境で観測した値**（2026-08-29）。製品として定められた上限か環境依存かは照会中で、容量計画の前提に置く前に確定させる必要があります |
| 再初期化中のクォータ | 強制されない。データの可用性には影響しない | AWS 確認（2026-08-29） |
| 再初期化の所要時間 | 目安なし。ルール数と負荷に依存 | AWS 確認（2026-08-29） |

> **AWS のブログに誤りがあります。** 2026-08-29 に自環境で実測しました。グループ単位のクォータ指定について、実際の挙動は次のとおりです。`-type group -target DOMAIN\Group` は**エラー**になります。`-type user -target <グループ SID>` は**作成自体は成功しますが、グループ制限として機能しません**。ブログの記載どおりに構成すると、エラーにならないまま制限が効かない状態になります。修正を依頼済みですが、公開されるまではブログの構文を信用しないでください。

### スループット変更と CIFS オプション

スループットキャパシティを変更しても CIFS オプションは維持されます（AWS が自社環境で確認、2026-08-29）。設定が戻るという資料は確認されていません。
...
| [書き込み操作の検証計画](../../solutions/amplify-portal/docs/write-verification-plan.md) | 未実行の書き込み操作の前提と影響 |
