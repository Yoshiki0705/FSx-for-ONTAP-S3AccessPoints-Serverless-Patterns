...
**実行可能なコマンドの一覧は公開されていません**（AWS 確認、2026-08-29）。ただし列挙する手段があります。
...
コマンドによって要求される権限が異なります（AWS 確認、2026-08-29）。
...
> **AWS のブログに誤りがあります**（AWS 確認、2026-08-29）。グループ単位のクォータ指定について、正しい挙動は次のとおりです。`-type group -target DOMAIN\Group` は**エラー**になります。`-type user -target <グループ SID>` は**作成自体は成功しますが、グループ制限として機能しません**。ブログの記載どおりに構成すると、エラーにならないまま制限が効かない状態になります。修正を依頼済みですが、公開されるまではブログの構文を信用しないでください。
