...
**重要**: Guard Hook のエイリアスは `FSxNS3AP::Guard::Hook` で固定。
パターン制約: `^(?!(?i)aws)[A-Za-z0-9]{2,64}::[A-Za-z0-9]{2,64}::[A-Za-z0-9]{2,64}$`
（ハイフン不可、`AWS` プレフィックス不可）

### Step 2: 動作確認

```bash
# Hook の登録状態確認（Hooks 一覧）
aws cloudformation list-types \
  --type HOOK \
  --visibility PRIVATE \
  --region ap-northeast-1 \
  --query "TypeSummaries[?contains(TypeName, 'Guard')]"
...
### Step 3: テストデプロイ（Hook 動作確認）

Guard Hook が WARN モードで動作していることを確認:
...
Resources:
  UnsafeBucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub "test-no-encryption-${AWS::AccountId}"
      # BucketEncryption が未設定 → Guard Hook で警告される
```

```bash
# テストデプロイ（WARN モードなら警告付きで成功する）
aws cloudformation deploy \
  --template-file test-violation.yaml \
  --stack-name test-guard-violation \
  --region ap-northeast-1

# イベントログで Hook invocation を確認
aws cloudformation describe-stack-events \
  --stack-name test-guard-violation \
  --query "StackEvents[?contains(ResourceStatusReason || '', 'Hook')]" \
  --region ap-northeast-1
# 期待される出力: "Hook invocations complete. Resource creation initiated"
...
**実際の検証結果（2026-05-09）:**
- Hook invocation 確認: `"Hook invocations complete. Resource creation initiated"`
- WARN モードのため、ルール違反があってもデプロイは続行される
- FAIL モードに切り替えると、ルール違反時にデプロイがブロックされる
