...
| 業界 | 参照 UC | PoC Objective の例 |
|------|---------|-------------------|
| Legal / Compliance | UC1 | ファイルサーバー監査・コンプライアンスレポート自動化 |
| Public Sector | UC16 | 公文書アーカイブの自動分類・FOIA 対応迅速化 |
| Healthcare | UC5 | DICOM 画像の自動分類・匿名化 |
| Enterprise Integration | SAP/ERP Adjacent | IDoc/HULFT/EDI ランディングゾーンの自動処理 |
| Financial Services | UC2, UC14 | 請求書 OCR・保険査定レポート自動化 |
| Manufacturing | UC3 | IoT ログ・品質検査画像の異常検知 |
