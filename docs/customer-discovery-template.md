...
## 8. Cache / Clone / RAG ガバナンス質問

FlexCache / FlexClone / RAG パターンを検討する場合の追加質問:

| # | 質問 | 回答形式 |
|---|------|---------|
| 1 | ワークロードはキャッシュまたはクローンデータを使用しますか？ | はい / いいえ |
| 2 | 正本（authoritative source）はどのボリュームですか？ | ボリューム名 + データオーナー |
| 3 | キャッシュ/クローンボリュームにアクセスできるチームは？ | チーム名 + 承認者 |
| 4 | ルート変更やフェイルオーバー判断を承認できるのは誰ですか？ | Decision owner + Approval role |
| 5 | ルート変更を監査イベントとして記録すべきですか？ | はい / いいえ + 保持期間 |
| 6 | RAG 前処理でファイルシステム権限（ACL / グループ）を保持する必要がありますか？ | はい / いいえ |
| 7 | AI 生成出力を外部共有・業務利用前にレビューするのは誰ですか？ | レビュー担当者 |
| 8 | レビュー証跡・ルート変更ログの保持期間は？ | 期間 + 削除権限者 |
| 9 | レビュー証跡を削除・変更できるのは誰ですか？ | 権限者 |
