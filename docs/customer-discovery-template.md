...
🌐 **Language / 言語**: 日本語 | [English](customer-discovery-template.en.md)

## 概要

本テンプレートは、導入検討の初回ヒアリングで FSx for ONTAP S3 Access Points サーバーレスパターンの適用可否を判断するための質問項目を整理したものです。
...
PoC 開始時に、使用するデータの性質を確認してください:

| # | 質問 | 回答 |
|---|------|------|
| 1 | PoC では合成データ、匿名化データ、本番類似データのいずれを使用しますか？ | |
| 2 | サンプルデータの使用を承認するのは誰ですか？ | |
| 3 | サンプルデータに個人情報、規制対象情報、機密情報は含まれますか？ | |
| 4 | テスト前にマスキングや匿名化が必要ですか？ | |
| 5 | PoC 終了後のテストデータ削除方針は？ | |
