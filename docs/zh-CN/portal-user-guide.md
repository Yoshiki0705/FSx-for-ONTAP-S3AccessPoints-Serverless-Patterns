...
没有专用应用。在手机浏览器中打开**与桌面相同的 URL**（已在 iOS Safari 和 Android Chrome
上验证）。

<img src="../../solutions/amplify-portal/docs/screenshots/portal-files-mobile-dark.png" alt="手机上的文件列表（深色主题）" width="300">

**步骤**

1. 在浏览器中打开管理员提供的 URL
2. 使用邮箱和密码登录（若启用 MFA，还需输入 TOTP 验证码）。密码管理器的自动填充可正常使用
3. 屏幕顶部有 **☰**、主题切换、语言切换和退出登录（⏻）。侧边栏默认隐藏
4. 点击 **☰** 会在正文上方打开导航。选择某个板块后会自动关闭；若不选择而想关闭，点击变暗的区域
5. 要打开文件，点击该行的图标（📄 / 🖼️ / 📕 / 📝）。预览以从屏幕底部升起的面板形式打开，
   用 **✕** 关闭
6. 要同时处理多个文件，点击每行左侧的复选框。所选数量和可用操作会显示在列表上方
...
|------|---------|
| 侧边栏 | 覆盖正文的抽屉，用 **☰** 开关 |
| 大小 / 修改时间列 | 因宽度不足而省略；仍可按名称排序 |
| 邮箱地址 | 隐藏（退出登录仅显示图标） |
| 文件预览 | 从底部升起的面板（最多占屏幕 70%）。PDF 横屏阅读更方便 |
| AI 助手面板 | 从右侧打开的抽屉 |

> **关于主题中的 🖥️**：它不是"切换到电脑版"按钮。三个选项是 ☀️ 浅色、🌙 深色和
> 🖥️ **跟随设备设置**，选择 🖥️ 会跟随 iOS 或 Android 的外观设置（包括夜间自动切换）。

> **流量提示**：文件夹 ZIP 下载会传输该文件夹下的全部内容。使用移动网络时，请先确认文件数量和大小。
