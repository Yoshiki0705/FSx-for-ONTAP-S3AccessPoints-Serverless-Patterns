...
FSx for ONTAP ボリューム上のファイルを **Web ブラウザから閲覧・処理指示・結果確認**するためのフロントエンドには、複数のアーキテクチャ選択肢があります。本ドキュメントでは、AWS Amplify Gen2、Nextcloud、カスタムビルド（CDK + フレームワーク）の3つを比較し、チームの状況に応じた選び方を示します。
...
| **S3 AP Presigned URL** | 非対応（Lambda プロキシ） | 非対応（Lambda プロキシ） | 非対応（Lambda プロキシ） |
...
- Presigned URL は FSx for ONTAP S3 AP で非対応のため、Nextcloud はサーバープロセス経由でファイルダウンロードをプロキシ
...
> **Governance note**: S3 AP は Presigned URL を非対応のため、ファイルコンテンツは必ずサーバーサイドコンポーネント（Lambda または Nextcloud サーバー）を経由します。これにより、データ滞留制御がアプリケーション層で担保されます。
...
A: FSx for ONTAP S3 AP は Presigned URL を非対応です。すべてのファイルコンテンツはサーバーサイドプロキシ（Amplify/カスタムでは Lambda、Nextcloud ではサーバープロセス）を経由します。これはデータガバナンスの観点ではメリットです — コンテンツが必ず制御可能なレイヤーを通過するため。
...
*最終更新: 2025-07 | 対象: FSx for ONTAP S3 AP Serverless Patterns v1.x*
