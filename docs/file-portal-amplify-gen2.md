...
**最初に確認すること: 作らずに済むかもしれません。** 用件が「ファイル転送」なら AWS Transfer Family が設定のみで SFTP / FTPS / FTP を提供し、FSx for ONTAP S3 AP 経由で同じボリュームを参照します。ブラウザ UI が要るなら Transfer Family web apps がマネージドなポータルを提供しますが、対象ストレージは Amazon S3 バケットです。詳しくは[そもそも作る必要があるか](#そもそも作る必要があるか)を先に読んでください。以下の 3 択は、そこで「作る」と判断した場合の比較です。
