...
전용 앱은 없습니다. **데스크톱과 같은 URL** 을 스마트폰 브라우저에서 엽니다
(iOS Safari, Android Chrome 에서 확인).

<img src="../../solutions/amplify-portal/docs/screenshots/portal-files-mobile-dark.png" alt="스마트폰의 파일 목록(다크 테마)" width="300">

**절차**

1. 관리자에게 받은 URL 을 브라우저에서 엽니다
2. 이메일과 비밀번호로 로그인합니다(MFA 가 켜져 있으면 TOTP 코드도). 비밀번호 관리 앱의
   자동 입력을 그대로 사용할 수 있습니다
3. 화면 상단에 **☰**, 테마 전환, 언어 전환, 로그아웃(⏻) 이 있습니다. 사이드바는 처음에는
   숨겨져 있습니다
4. **☰** 를 탭하면 내비게이션이 본문 위에 열립니다. 섹션을 선택하면 자동으로 닫히고,
   선택하지 않고 닫으려면 어두워진 영역을 탭합니다
5. 파일을 열려면 행의 아이콘(📄 / 🖼️ / 📕 / 📝)을 탭합니다. 미리 보기는 화면 아래에서
   올라오는 시트로 열리고 **✕** 로 닫습니다
6. 여러 파일을 함께 다루려면 각 행 왼쪽의 체크박스를 탭합니다. 선택 개수와 사용 가능한
   작업이 목록 위에 표시됩니다
...
| 항목 | 스마트폰에서 |
|------|------------|
| 사이드바 | 본문 위에 겹치는 드로어. **☰** 로 열고 닫음 |
| 크기 / 수정 날짜 열 | 화면 폭이 부족해 생략. 이름으로는 정렬 가능 |
| 이메일 주소 | 숨김(로그아웃은 아이콘만) |
| 파일 미리 보기 | 화면 아래에서 올라오는 시트(최대 화면의 70%). PDF 는 가로 방향이 읽기 편합니다 |
| AI 어시스턴트 패널 | 오른쪽에서 열리는 드로어 |

> **테마의 🖥️ 에 대해**: "PC 화면으로 전환" 버튼이 아닙니다. ☀️ 라이트 / 🌙 다크 /
> 🖥️ **기기 설정에 맞춤** 의 3 가지 중 하나이며, 🖥️ 는 iOS 나 Android 의 화면 모드
> 설정(야간 자동 전환 포함)을 따릅니다.

> **데이터 사용량**: 폴더 ZIP 다운로드는 폴더 아래 전체를 전송합니다. 모바일 회선에서는
> 파일 수와 크기를 먼저 확인하세요.
...
A: 네. 절차는 시작하기의 "4. 스마트폰에서 사용하기" 에 있습니다.
