...
本ドキュメントでは、ワークロードアカウントの Lambda 関数が中央ストレージアカウントに配置された S3 Access Point（FSx for ONTAP）にセキュアにアクセスするためのクロスアカウントパターンを定義する。
...
- S3 Access Point が FSx for ONTAP ボリュームに対して作成済み
...
| S3 Access Point | Storage | FSx for ONTAP ボリュームへのアクセスゲートウェイ |
| FSx for ONTAP Volume | Storage | 実データの格納先 |
...
FSx for ONTAP の Export Policy は S3 Access Point 経由のアクセスに対して以下の設定が必要:
