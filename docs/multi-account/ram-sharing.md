# AWS RAM による FSx for ONTAP リソース共有
...
### FSx for ONTAP 関連リソースの RAM 対応状況
...
│  │  • VPC Subnets (FSx for ONTAP 配置サブネット)               │   │
│  │  • Transit Gateway                                       │   │
...
│  • Transit Gateway 経由で FSx for ONTAP に接続                     │
└─────────────────────────────────────────────────────────────────┘
```
...
FSx for ONTAP S3 Access Points が RAM 非対応のため、以下の代替パターンを使用する:
...
```
┌─────────────────────────────────────────────────────────┐
│ FSx for ONTAP コスト（ストレージアカウント負担）            │
│  • ファイルシステム料金                                 │
│  • ストレージ容量料金                                   │
│  • バックアップ料金                                     │
├─────────────────────────────────────────────────────────┤
...
AWS は継続的に RAM 対応リソースを拡大している。FSx for ONTAP リソースが RAM 対応になった場合:
