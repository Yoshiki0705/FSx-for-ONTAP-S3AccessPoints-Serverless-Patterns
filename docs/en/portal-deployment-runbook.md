...
| **cdk-nag blocks sandbox deploy** | Only if cdk-nag is made always-on (off by default, so normally cannot occur) | Deploy without `CDK_NAG=1`. Inspect nag separately with `npm run nag` |
