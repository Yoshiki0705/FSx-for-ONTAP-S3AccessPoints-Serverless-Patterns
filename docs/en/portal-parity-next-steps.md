...
- [The same four features on three foundations](portal-parity-four-features.md) built exactly
  ...
  requires**, phase by phase, so a reader can go as far as their own requirements need and stop.
- There are six phases. **Phases 1 to 3 need no VPC attachment.** From phase 4 the Amazon FSx for
...
- **The first thing to settle is not how many phases but the scope of writes and whether an audit
  trail is required.** Staying read-only through phase 3 and allowing writes from phase 1 differ
  enormously in what it costs to change your mind.
- Cost steps up discontinuously at phase 4, because a VPC endpoint or a NAT gateway becomes
  necessary and bills regardless of traffic. See [cost per phase](#cost-per-phase).
- At every phase, **decide early to confine writes to a prefix**. Narrowing it later means moving
  objects that have already been written.

## Scope

**Covered**: the phases beyond the four features, what each newly requires (permissions,
networking, resources, operational work), the direction cost moves, and which phases can be
skipped.

**Not covered**: implementation code for each phase (linked to the relevant pattern in this
...
| Phase | Addition | VPC attachment | Newly required |
|---|---|---|---|
...
| 5 | ONTAP operations (snapshot, FlexClone, locking) | **Yes** | Everything in phase 4, plus approval for irreversible operations and an audit trail |
...
Phase 4 is the boundary. **Everything up to phase 3 is satisfied by the S3 Access Point alone**;
from phase 4 the ONTAP management plane has to be reachable. What can actually reach that plane is
in [FSx for ONTAP management interfaces](fsx-ontap-management-interfaces.md).

## Phase 1. Paging, search, sorting
...
## Phase 2. Download and shareable links
...
S3 access point. Reaching phase 4 increases **not the number of functions but the number of places
they live**.

**This is where cost steps up discontinuously** (see [cost per phase](#cost-per-phase)).

**Decide first**: whether to stop at information or go on to phase 5. Display alone is satisfied by
a read-only ONTAP user, which makes the permission design far lighter.

## Phase 5. ONTAP operations

Snapshot creation, FlexClone, volume locking — operations that change state.

**Newly required**: everything in phase 4, plus
...
([SnapLock pitfalls](../agent/pitfalls-snaplock.md)). Worth reading at the point phase 5 is being
...
| Phase | Nature of the increment | Traffic-dependent |
|---|---|---|
...
| 5 | As phase 4, plus FlexClone and snapshot capacity | Partly |
| 6 | Per-token inference charges | **Yes** |

**Phase 4 is the step.** Phases 1 to 3 charge for what is used; from phase 4 onward something
charges for merely existing. If phase 4 was built to try something out, deciding when to remove it
is part of the work.
...
| 4 (ONTAP information) | **No** | It is the prerequisite for phase 5; without a path to the management plane there is no state to read |
...
**Phase 4 is the one that is hard to add later**, because it concerns network placement and
function separation, which means rewriting the phase 1-3 implementation. If phase 5 or later is a
possibility, **split functions into "touches the S3 access point" and "touches ONTAP" from the
start** and adding phase 4 later becomes wiring rather than restructuring.

## What each foundation is like to extend

All six phases are reachable on any of the three. What differs is the nature of the work.
