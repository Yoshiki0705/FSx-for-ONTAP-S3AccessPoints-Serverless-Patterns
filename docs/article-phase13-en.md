...
**Trigger strategy matters.** Because native S3AP event notifications are not available, the repository provides POLLING (simplest), EVENT_DRIVEN (FPolicy-based, near-real-time; not native S3 bucket notifications), and HYBRID modes. Default is POLLING. **EVENT_DRIVEN applies only where writes arrive over NFS or SMB**: measured on 2026-08-26 against ONTAP 9.18.1P3D1, operations through an S3 access point raise no FPolicy notification and are not blocked even by a `mandatory` synchronous policy, so a write that arrives through the access point is picked up only by the polling half of HYBRID. See [the measured FPolicy / S3 access point coverage](https://github.com/Yoshiki0705/FSx-for-ONTAP-Observability-integrations/blob/main/docs/en/s3ap-monitoring-coverage-implications.md).
