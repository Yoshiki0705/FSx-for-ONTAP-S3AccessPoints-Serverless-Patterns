...
| データ | サンプル / 合成データ | 導入先管理の NAS データ |
| ガバナンス | デモラベルのみ | データ分類 + リネージ + 保持ポリシー |
| コスト | ~$0.10/実行 | + FSx for ONTAP インフラ (~$194/月 基本) |
| AI 評価 | テストデータでの動作確認 | ドメインバリデーションセットでの精度評価 |

> **Governance Caveat**: デモモードは技術検証用です。本番環境では必ず FSx for ONTAP S3 Access Points を使用し、適切な IAM ポリシー、ネットワーク設計、データ分類、人間レビュー閾値を定義してください。各国・地域の規制要件への適合は導入先の責任で検証する必要があります。
