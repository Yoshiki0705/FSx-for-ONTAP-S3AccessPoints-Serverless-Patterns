...
本ドキュメントは、この論点を**公開ドキュメントの記述から整理したもの**です。**実機での E2E 測定は行っていません**（末尾の補足を参照）。
...
以下 3 点は、末尾に挙げた公開ドキュメントの記述から導いたものです。**当方で測ったものではありません。**
...
> **検証範囲に関する補足**: 上記 3 点は公開ドキュメントの記述からの導出で、当プロジェクトで
> 実機による E2E 測定を実施したものではありません（`open`）。本番移行の前に、対象データの一部で
