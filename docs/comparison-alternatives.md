...
「なぜ FSx ONTAP S3 Access Points + Lambda なのか？」という質問に対する
技術的な比較資料です。

## 比較マトリクス

| 観点 | FSx ONTAP S3 AP + Lambda | EFS + Lambda | EC2 NFS マウント | DataSync → S3 + Lambda |
|------|:---:|:---:|:---:|:---:|
...
| **コスト (ストレージ)** | FSx ONTAP (既存) | EFS 追加 | FSx ONTAP (既存) | S3 追加 |
...
### FSx ONTAP S3 AP + Lambda を選ぶべき場合
