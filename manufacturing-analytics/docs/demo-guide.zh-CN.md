...
**解说要点**:
> 当传感器数据积累到文件服务器时，分析管道会自动启动。

**Key Visual**: 数据文件部署 → 工作流启动
...
**Key Visual**: Athena 查询结果 — 按生产线、工序分类的异常分布

### Section 5: Report & Action（3:45–5:00）

**解说要点**:
> AI 生成质量检查报告。提出异常的根本原因候选和推荐对应措施。
...
| Report | 3:45–5:00 | "AI 提出根本原因候选和对应措施" |
...
UC3 manufacturing-analytics 被分类为 **Pattern A: Native S3AP Output**
...
与 Phase 7 UC15/16/17 和 UC6/11/14 的演示采用相同方针，以**最终用户在日常业务中实际
...
![UC3 Step Functions 图表视图 (SUCCEEDED)](../../docs/screenshots/masked/uc3-demo/step-functions-graph-succeeded.png)

![UC3 Step Functions 图表 (展开视图)](../../docs/screenshots/masked/uc3-demo/step-functions-graph-expanded.png)

![UC3 Step Functions 图表 (缩放 — 各步骤详细)](../../docs/screenshots/masked/uc3-demo/step-functions-graph-zoomed.png)
