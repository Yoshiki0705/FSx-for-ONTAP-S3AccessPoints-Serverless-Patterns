...
| メトリクス | 測定方法 |
|-----------|--------|
| 映像/LiDAR ファイル処理件数 / 実行 | CloudWatch Metrics / Step Functions 実行結果 |
| 品質チェック通過率 | CloudWatch Metrics / Step Functions 実行結果 |
| アノテーション前処理スループット | CloudWatch Metrics / Step Functions 実行結果 |
| SageMaker 推論レイテンシ | CloudWatch Metrics / Step Functions 実行結果 |
