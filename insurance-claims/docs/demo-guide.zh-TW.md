...
本示範展示了從事故照片進行損害評估與保險理賠報告自動生成的流程。透過影像分析進行損害評估與 AI 報告生成，提升理賠流程效率。

**示範核心訊息**:AI 自動分析事故照片,即時評估損害程度並生成保險理賠報告。
...
**旁白要點**:
> 每月 100 件以上的保險理賠申請。每個案件需確認多張事故照片,評估損害程度並製作報告。手動處理無法負荷。
...
**旁白要點**:
> AI 分析照片並檢測損傷位置。識別損傷類型(凹陷、刮痕、破損)與部位(保險桿、車門、擋泥板等)。

**Key Visual**: 損傷檢測結果、部位對應
...
| Upload | 0:45–1:30 | 「上傳照片即啟動自動評估」 |
| Detection | 1:30–2:30 | 「AI 自動檢測損傷位置與類型」 |
...
| Lambda (Image Analyzer) | 透過 Bedrock/Rekognition 進行損傷檢測 |
...
理賠案件目錄結構內瀏覽損害評估 JSON・OCR 結果・理賠報告
...
- **Rekognition**: 事故照片檢測到 `Maroon` 90.79%, `Business Card` 84.51% 等
