...
| メトリクス | 測定方法 |
|-----------|--------|
| アセットメタデータ抽出件数 / 実行 | CloudWatch Metrics / Step Functions 実行結果 |
| レンダリングジョブ前処理時間 | CloudWatch Metrics / Step Functions 実行結果 |
| アセット検索時間の短縮率 | CloudWatch Metrics / Step Functions 実行結果 |
| 処理あたりのコスト | CloudWatch Metrics / Step Functions 実行結果 |
| ファイルサイズ別処理スループット | CloudWatch Metrics / Step Functions 実行結果 |
