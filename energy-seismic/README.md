...
| メトリクス | 測定方法 |
|-----------|--------|
| SEG-Y ファイル処理件数 / 実行 | CloudWatch Metrics / Step Functions 実行結果 |
| メタデータ抽出精度 | CloudWatch Metrics / Step Functions 実行結果 |
| 異常検知アラート精度 | CloudWatch Metrics / Step Functions 実行結果 |
| レポート生成時間 | CloudWatch Metrics / Step Functions 実行結果 |
