...
| メトリクス | 測定方法 |
|-----------|--------|
| GDS/OASIS ファイル処理件数 / 実行 | CloudWatch Metrics / Step Functions 実行結果 |
| DRC 集計レポート生成時間 | CloudWatch Metrics / Step Functions 実行結果 |
| Bedrock 設計レビュー生成時間 | CloudWatch Metrics / Step Functions 実行結果 |
| Athena クエリ応答時間 | CloudWatch Metrics / Step Functions 実行結果 |
